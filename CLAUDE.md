...
39. **npm `overrides` with `>=` is a footgun** — `"path-to-regexp": ">=0.1.13"` tells npm "any version ≥ 0.1.13", which resolves to the *highest* matching version (e.g. 8.x). That breaks Express 4, which requires the 0.1.x function-default export. Error at boot: `TypeError: pathRegexp is not a function` inside `express/lib/router/layer.js`. **Always use EXACT pins in `overrides`** (`"path-to-regexp": "0.1.13"`) unless you explicitly want a range. `server/package.json` pins `path-to-regexp` and `dompurify` defensively. Same rule for `picomatch`, `lodash`, etc.
