...
21. **⚠️ D1 LIKE/GLOB pattern cap — 50 BYTES, and it is a THIRD separate limit** from the column and bound-parameter caps above. SQLite checks `SQLITE_LIMIT_LIKE_PATTERN_LENGTH` against `sqlite3_value_bytes()`, so the unit is encoded UTF-8 bytes, **not characters**. Exceed it and the query **throws** `D1_ERROR: LIKE or GLOB pattern too complex` — it does not match loosely.
    - **Never hand-roll `` `%${term.slice(0, 48)}%` ``.** `.slice` counts UTF-16 units, which equals bytes only for ASCII. Measured in Miniflare 2026-09-15: a 50-byte pattern succeeds, 51 throws — and **25 accented characters is 52 bytes but only 27 UTF-16 units**, so it sails through a `.slice(0, 48)` guard straight into the throw. An address with a diacritic, an em dash, or an emoji was enough.
    - **Use `likePattern()` from [`src/utils/d1Like.ts`](src/utils/d1Like.ts)**, which owns the budget in bytes, never splits a multi-byte character, and supports `{ escape, match: 'contains'|'prefix'|'suffix', caseFold }`. `cappedLikePattern()` and `codedLike()` in [`src/utils/searchText.ts`](src/utils/searchText.ts) delegate to it, so there is exactly one implementation of the cap.
    - **`escape: true` requires `ESCAPE '\\'` in the SQL, and escaping must precede the measurement** — each escaped metacharacter costs an extra byte, so 48 literal `%` become 96 bytes. The trim must also drop whole escape PAIRS: a dangling lone `\\` is an **error** under `ESCAPE '\\'`, not a literal backslash. `likePattern` does both; a `.replace(...).slice(...)` in either order does not.
    - **A prefix/suffix pattern spends ONE wildcard, not two**, so its needle budget is one byte larger. `statutes.ts` mixed `` `${safe}%` `` and `` `%${safe}%` `` off one 48-byte budget.
    - **It does NOT reliably reach `error_log`, and the silent variant is worse.** Routes that wrap the query in a catch degrade to an empty result set — `citations.ts`'s statute autocomplete returned `{ data: [] }`, indistinguishable from "no such statute". Don't conclude "no errors" from a clean `error_log`.
    - **The cap IS reproducible locally** (Miniflare, `vitest.workers.config.mts`) — `test-workers/likePatternD1Cap.test.ts` pins every helper against real D1. An older comment in `searchText.ts` claimed it could only surface against remote D1; that was wrong and would talk you out of writing the test.
    - Prefer `containsClause()` / `containsAnyClause()` (`instr()`) for new substring search — `instr()` has no pattern cap at all and matches literally, so it needs no escaping.
