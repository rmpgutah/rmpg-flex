...
// ----------------------------------------------------------
// Nearest-beat fallback (when point is outside all polygons)
// ----------------------------------------------------------

/** Calculate the centroid of a feature's first sub-polygon outer ring. */
...
/**
 * Find the nearest beat to a coordinate (within 1.25 miles).
 * Used as a fallback when the point falls outside all beat polygons.
 * Returns the match plus distance info, or null if nothing is close enough.
 */
export function findNearestBeat(
  lat: number,
  lng: number
): (BeatMatch & { exact: boolean; distance_mi: number }) | null {
  const features = loadBeats();
  ...
  return {
    ...
    beat_number: p.beat_number,
    exact: false,
    distance_mi: Math.round(bestDist * 100) / 100,
  };
}
