...
function getEncryptionKey(): Buffer {
  ...
  const keyHex = config.twoFactor.encryptionKey;
...
export function generateBackupCodes(count: number = config.twoFactor.backupCodeCount): string[] {
  const codes: string[] = [];
...
export function hashBackupCode(code: string): string {
  ...
  return bcryptjs.hashSync(normalized, 8);
}
