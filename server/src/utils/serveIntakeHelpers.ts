...
export function buildNotesNarrative(i: NotesInput): NotesEntry[] {
  ...
  // ═══════════════════════════════════════════════════════════
  // ═══════════════════════════════════════════════════════════
  // NOTE 0 — 📋 CASE SUMMARY — DETAILED NARRATIVE
  //   ONE consolidated, top-of-file narrative that tells the full
  //   story of what the case is about in plain English. Combines:
  //     - Synopsis (what / parties / category / money / urgency)
  //     - Case narrative (Who / What / Where / When / Why from the
  //       Complaint with operative facts + causes of action + flags)
  //     - Bottom-line briefing footer (deadlines, attempts plan)
  //   This is the first note an officer or supervisor sees and is
  //   designed to be readable as a single narrative without
  //   bouncing between sub-notes. Other notes (BRIEFING / PACKET /
  //   DOSSIER) remain for drill-down operational use.
  // ═══════════════════════════════════════════════════════════
  const summaryLines: string[] = [];
  summaryLines.push('═══════════════════════════════════════════════');
  summaryLines.push('  CASE SUMMARY — DETAILED NARRATIVE');
  summaryLines.push('═══════════════════════════════════════════════');
  summaryLines.push('Auto-generated by Serve Intake. Verify against the');
  summaryLines.push('underlying PDFs before relying on for affidavits.');
  summaryLines.push('');

  // Section 1 — Executive synopsis
  if (i.caseSynopsisText) {
    summaryLines.push('───  SYNOPSIS  ───');
    summaryLines.push(i.caseSynopsisText.trim());
    summaryLines.push('');
  }

  // Section 2 — Detailed Who/What/Where/When/Why narrative.
  // Strip the upstream header line so the consolidated note doesn't
  // double-print "CASE NARRATIVE - Detailed review of the Complaint"
  // — that's already implied by the section we're building.
  if (i.caseNarrativeText) {
    summaryLines.push('───  COMPLAINT REVIEW (Who / What / Where / When / Why)  ───');
    const cleaned = i.caseNarrativeText
      .replace(/^CASE NARRATIVE[^\n]*\n/, '')
      .replace(/^Auto-generated[^\n]*\n/, '')
      .replace(/^\n+/, '');
    summaryLines.push(cleaned.trim());
    summaryLines.push('');
  }

  // Section 3 — Bottom-line operational footer
  summaryLines.push('───  BOTTOM LINE  ───');
  summaryLines.push(`SERVE: ${docList || 'unspecified documents'} on the named defendant.`);
  if (i.dueDate) {
    const urgency = i.daysRemaining > 0
      ? `${i.daysRemaining} day(s) remaining`
      : i.daysRemaining === 0
        ? 'DUE TODAY'
        : `${Math.abs(i.daysRemaining)} day(s) PAST DUE`;
    summaryLines.push(`DEADLINE: ${i.dueDate}  (${urgency}).`);
  } else {
    summaryLines.push('DEADLINE: none stated — proceed under standard 3-day diligence rules.');
  }
  summaryLines.push(`RESPONSE WINDOW: ${i.responseDeadlineDays || 21} day(s) after service to file an Answer.`);
  if (i.recommendedAttempts.length > 0) {
    const attempts = i.recommendedAttempts
      .map((a, idx) => `(${idx + 1}) ${a.label}${a.weekend ? ' WKND' : ''}`)
      .join(' → ');
    summaryLines.push(`PLAN: ${attempts}.`);
  }
  if (i.serviceWindows) {
    summaryLines.push(`WINDOWS: ${up(i.serviceWindows)}.`);
  }
  summaryLines.push('');
  summaryLines.push(`[Auto-generated ${i.timestamp}]`);

  const summaryNote = summaryLines.join('\n');

  const notes: NotesEntry[] = [
    { text: summaryNote },        // 0 — CASE SUMMARY (top-of-file narrative)
    { text: briefingNote },       // 1 — OFFICER BRIEFING
    { text: packetNote },         // 2 — CASE PACKET
  ];
  if (i.caseNarrativeText) {
    notes.push({ text: i.caseNarrativeText });  // 3 — CASE NARRATIVE (detail drill-down)
  }
  notes.push({ text: dossierNote });            // 4 — SUBJECT & ADDRESS DOSSIER
  return notes;
}
