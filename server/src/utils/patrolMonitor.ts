    ...
    if (timeSinceLastScan > overdueThreshold) {
      ...
      const existingNotif = db.prepare(`
        ...
        WHERE entity_type = 'patrol_checkpoint' AND entity_id = ?
          ...
          AND created_at >= datetime('now', '-${cp.scan_required_interval_minutes} minutes')
      `).get(cp.id) as any;
      ...
      const body = `Checkpoint "${cp.name}" (${cp.location_description || 'N/A'}) is ${overdueMinutes}min overdue. Required interval: ${cp.scan_required_interval_minutes}min.`;
