  ...
  api.get('/departments', async (c) => {
    try {
      ...
      const departments = await db.prepare(`
        SELECT d.*, u.full_name as manager_name, pd.name as parent_name
        FROM departments d
      ...
      return c.json({ error: 'Failed to get departments', code: 'GET_DEPARTMENTS_ERROR' }, 500);
    }
  });

  // POST /api/admin/departments — Create department
  api.post('/departments', requireRole('admin', 'manager'), async (c) => {
    try {
      const db = new D1Db(c.env.DB);
      const user = c.get('user');
      const { name, code, description, parent_id, manager_id, is_active } = await c.req.json();
      if (!name) return c.json({ error: 'name is required', code: 'NAME_IS_REQUIRED' }, 400);
      const now = localNow();
      const result = await db.prepare(`
        INSERT INTO departments (name, code, description, parent_id, manager_id, is_active, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `).run(name, code || null, description || null, parent_id || null, manager_id || null,
      ...
      if (err?.message?.includes('UNIQUE constraint')) {
        return c.json({ error: 'A department with this name or code already exists', code: 'A_DEPARTMENT_WITH_THIS' }, 409);
      }
      return c.json({ error: 'Failed to create department', code: 'CREATE_DEPARTMENT_ERROR' }, 500);
    }
  });

  // PUT /api/admin/departments/:id — Update department
  api.put('/departments/:id', requireRole('admin', 'manager'), async (c) => {
    try {
      const db = new D1Db(c.env.DB);
      const id = paramNum(c.req.param('id'));
      const existing = await db.prepare('SELECT * FROM departments WHERE id = ?').get(id) as any;
      if (!existing) return c.json({ error: 'Department not found', code: 'DEPARTMENT_NOT_FOUND' }, 404);
      const body = await c.req.json();
      const updatable = ['name', 'code', 'description', 'parent_id', 'manager_id', 'is_active'];
      const sets: string[] = [];
      const vals: any[] = [];
      ...
      vals.push(localNow());
      vals.push(id);
      await db.prepare(`UPDATE departments SET ${sets.join(', ')} WHERE id = ?`).run(...vals);
      ...
      return c.json({ error: 'Failed to update department', code: 'UPDATE_DEPARTMENT_ERROR' }, 500);
    }
  });

  // DELETE /api/admin/departments/:id — Delete department
  api.delete('/departments/:id', requireRole('admin'), async (c) => {
    try {
      const db = new D1Db(c.env.DB);
      const id = paramNum(c.req.param('id'));
      const existing = await db.prepare('SELECT * FROM departments WHERE id = ?').get(id) as any;
      if (!existing) return c.json({ error: 'Department not found', code: 'DEPARTMENT_NOT_FOUND' }, 404);
      const userCount = await db.prepare("SELECT COUNT(*) as count FROM users WHERE department = ?").get(existing.name) as any;
      if (userCount?.count > 0) {
        return c.json({ error: `Cannot delete department with ${userCount.count} assigned user(s)` }, 400);
      }
      const childCount = await db.prepare('SELECT COUNT(*) as count FROM departments WHERE parent_id = ?').get(id) as any;
      if (childCount?.count > 0) {
        return c.json({ error: `Cannot delete department with ${childCount.count} child department(s)` }, 400);
      }
      await db.prepare('DELETE FROM departments WHERE id = ?').run(id);
      return c.json({ success: true });
    } catch (err: any) {
      return c.json({ error: 'Failed to delete department', code: 'DELETE_DEPARTMENT_ERROR' }, 500);
    }
  });
