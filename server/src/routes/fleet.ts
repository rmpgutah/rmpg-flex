...
router.get('/:id/cost-timeline', (req: Request, res: Response) => {
  try {
    ...
    const maint = db.prepare(`
      SELECT id, service_date, type, description, cost
      FROM fleet_maintenance
      WHERE vehicle_id = ? AND cost IS NOT NULL
    `).all(id) as any[];
    for (const m of maint) {
      const date = String(m.service_date).slice(0, 10);
      if (!dateFilter(date)) continue;
      entries.push({
        date,
...
router.get('/cost-analytics/overview', (req: Request, res: Response) => {
  try {
    ...
    const maint = db.prepare(`
      ...
      FROM fleet_maintenance WHERE service_date >= ? AND cost IS NOT NULL
    `).get(sinceDate) as any;
    const accessories = db.prepare(`
      SELECT COALESCE(SUM(cost), 0) AS total, COUNT(*) AS events
    ...
    const vehicleRows = db.prepare(`
      ...
      LEFT JOIN (SELECT vehicle_id, SUM(cost) AS cost FROM fleet_maintenance WHERE service_date >= ? AND cost IS NOT NULL GROUP BY vehicle_id) m ON m.vehicle_id = fv.id
    ...
    const monthlyTrend = db.prepare(`
      SELECT month, SUM(cost) AS cost FROM (
        ...
        UNION ALL
        SELECT strftime('%Y-%m', service_date) AS month, SUM(cost) AS cost FROM fleet_maintenance WHERE service_date >= ? AND cost IS NOT NULL GROUP BY month
        UNION ALL
    ...
    const perVehicleMonthly = db.prepare(`
      SELECT vehicle_id, month, SUM(cost) AS cost FROM (
        ...
        UNION ALL
        SELECT vehicle_id, strftime('%Y-%m', service_date) AS month, SUM(cost) AS cost
          FROM fleet_maintenance WHERE service_date >= ? AND cost IS NOT NULL GROUP BY vehicle_id, month
        UNION ALL
