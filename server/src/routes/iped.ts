...
router.put('/config', requireRole('admin'), (req: Request, res: Response) => {
  try {
    ...
    auditLog(req, 'iped_config_updated', 'integration', 0, 'IPED configuration updated');
    broadcastAdminUpdate({ type: 'iped_config_updated' });
...
router.delete('/config', requireRole('admin'), (req: Request, res: Response) => {
  try {
    ...
    auditLog(req, 'iped_config_cleared', 'integration', 0, 'IPED configuration cleared');
    broadcastAdminUpdate({ type: 'iped_config_cleared' });
...
router.post('/jobs', requireRole('admin', 'manager'), async (req: Request, res: Response) => {
  try {
    ...
    auditLog(req, 'iped_job_created', 'iped_job', jobId,
      `IPED ${jobType} job created for ${inputPath}`);
    broadcastAdminUpdate({ type: 'iped_job_created', jobId, jobType });
...
router.post('/jobs/:id/cancel', requireRole('admin', 'manager'), (req: Request, res: Response) => {
  try {
    ...
    if (cancelled) {
      auditLog(req, 'iped_job_cancelled', 'iped_job', id, `IPED job #${id} cancelled`);
      broadcastAdminUpdate({ type: 'iped_job_cancelled', jobId: id });
    }
...
router.post('/hash-sets/import', requireRole('admin'), (req: Request, res: Response) => {
  try {
    ...
    auditLog(req, 'iped_hashset_imported', 'iped_hashset', 0,
      `Imported hash set "${setName}" (${category}): ${count} hashes`);
...
router.delete('/hash-sets/:name', requireRole('admin'), (req: Request, res: Response) => {
  try {
    const setName = req.params.name as string;
    const removed = removeHashSet(setName);

    auditLog(req, 'iped_hashset_removed', 'iped_hashset', 0,
      `Removed hash set "${setName}": ${removed} hashes removed`);
