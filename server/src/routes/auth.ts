...
router.post('/verify-2fa', mfaRateLimit, (req: Request, res: Response) => {
  try {
    ...
    if (totpRecord) {
      ...
      let secret: string;
      try {
        // Legacy format: iv:tag:ciphertext as single colon-separated string
        const [legacyIv, legacyTag, legacyCiphertext] = user.totp_secret_enc.split(':');
        secret = decryptSecretV2(legacyCiphertext, legacyIv, legacyTag);
      } catch (decryptErr: any) {
