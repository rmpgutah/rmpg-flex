...
function solarTimes(date: Date): { sunrise: Date; sunset: Date } {
  ...
  const solarNoon = 12 - SLC_LNG / 15 + 7;
...
// ============================================================
// In-memory cache for safety assessments
// ============================================================
...
export function mountMapSafetyRoutes(app: Hono<{ Bindings: Env; Variables: { user: JwtPayload } }>): void {
  const api = new Hono<{ Bindings: Env; Variables: { user: JwtPayload } }>();
  api.use('/*', authenticateToken);

  // 1. GET /api/map/safety/threat-assessment/:lat/:lng
  api.get('/threat-assessment/:lat/:lng', requireRole(...OP_ROLES), async (c) => {
    try {
      ...
      if (isNaN(lat) || isNaN(lng) || coordErr) {
        return c.json({ error: coordErr || 'Invalid coordinates', code: 'INVALID_COORDS' }, 400);
      }
      ...
      const cutoff90Row = await db.prepare(`SELECT datetime('now','localtime','-90 days') as v`).get() as any;
      const cutoff90Val = cutoff90Row?.v;

      const nearbyCalls = await db.prepare(`
      ...
      for (const call of nearbyCalls) {
        ...
        if (call.weapons_involved && within90) { weaponCalls90++; score += 8; }
        if (call.domestic_violence) { dvCalls++; score += 5; }
        if (call.injuries_reported) { injuryCalls++; score += 4; }
        if (call.drugs_involved) { drugCalls++; score += 3; }
      }
      ...
      if (injuryCalls > 0) factors.push(`${injuryCalls} call(s) with injuries reported within 500m`);
      if (drugCalls > 0) {
      ...
      const warrants = await db.prepare(`
        SELECT w.id, w.latitude, w.longitude, COALESCE(p.first_name || ' ' || p.last_name, '') as subject_name
        ...
        WHERE w.status = 'active' AND w.latitude IS NOT NULL AND w.longitude IS NOT NULL
          AND w.latitude BETWEEN ? AND ? AND w.longitude BETWEEN ? AND ?
        LIMIT 1000
      `).all(bb.minLat, bb.maxLat, bb.minLng, bb.maxLng) as any[];

      let activeWarrantsNearby = 0;
      for (const w of warrants) {
        if (haversineMeters(lat, lng, w.latitude, w.longitude) <= 500) { activeWarrantsNearby++; score += 10; }
      }
      ...
      const officerIncidents = await db.prepare(`
      ...
      await auditLog(db, c, 'safety_read', 'threat_assessment', 0, `Threat assessment at ${lat.toFixed(4)},${lng.toFixed(4)}: ${threat_level} (${score})`);
      return c.json(result);
    } catch {
      return c.json({ error: 'Failed to compute threat assessment', code: 'THREAT_ASSESSMENT_ERROR' }, 500);
    }
  });

  // 2. GET /api/map/safety/approach-routes/:lat/:lng
  api.get('/approach-routes/:lat/:lng', requireRole(...OP_ROLES), async (c) => {
    try {
      const db = new D1Db(c.env.DB);
      const lat = parseFloat(c.req.param('lat') || '');
      const lng = parseFloat(c.req.param('lng') || '');
      ...
      const crossStreets = await db.prepare(`
        SELECT DISTINCT cross_street, latitude, longitude, incident_type, weapons_involved, domestic_violence
        FROM calls_for_service
      ...
      const nearbyPersons = await db.prepare(`
      ...
      const dangerousCalls = await db.prepare(`
      ...
      for (const call of dangerousCalls) {
        ...
        const dir = cardinalDirection(lat, lng, call.latitude, call.longitude);
      ...
      const approaches = directions.map((dir) => {
        ...
        if (threatFromDir > 0) risk_notes.push(`${threatFromDir} dangerous call(s) reported to the ${dir}`);
        const streetsInDir = crossStreets.filter((cs: any) => cardinalDirection(lat, lng, cs.latitude, cs.longitude) === dir);
      ...
      for (const p of nearbyPersons) {
        if (p.caution_flags) nearby_hazards.push(`Flagged person: ${p.first_name} ${p.last_name} — ${p.caution_flags}`);
      }

      return c.json({ approaches, nearby_hazards: nearby_hazards.slice(0, 10) });
    } catch {
      return c.json({ error: 'Failed to compute approach routes', code: 'APPROACH_ROUTES_ERROR' }, 500);
    }
  });

  // 3. GET /api/map/safety/corridor-analysis
  api.get('/corridor-analysis', requireRole(...OP_ROLES), async (c) => {
    try {
      const db = new D1Db(c.env.DB);
      const lat1 = parseFloat(c.req.query('lat1') || '');
      const lng1 = parseFloat(c.req.query('lng1') || '');
      const lat2 = parseFloat(c.req.query('lat2') || '');
      const lng2 = parseFloat(c.req.query('lng2') || '');

      if ([lat1, lng1, lat2, lng2].some(isNaN)) {
        return c.json({ error: 'Invalid coordinates — provide lat1, lng1, lat2, lng2', code: 'MISSING_COORDS' }, 400);
      }
      const err1 = validateLatLng(lat1, lng1);
      const err2 = validateLatLng(lat2, lng2);
      if (err1 || err2) return c.json({ error: err1 || err2 || 'Invalid coordinates', code: 'INVALID_COORDS' }, 400);

      const corridorDist = haversineMeters(lat1, lng1, lat2, lng2);
      if (corridorDist > 50_000) return c.json({ error: 'Corridor too long (max 50km)', code: 'CORRIDOR_TOO_LONG' }, 400);
      ...
      const calls = await db.prepare(`
        SELECT latitude, longitude, incident_type, priority, weapons_involved, domestic_violence, injuries_reported
        FROM calls_for_service
        WHERE latitude IS NOT NULL AND longitude IS NOT NULL
          AND latitude BETWEEN ? AND ? AND longitude BETWEEN ? AND ?
          AND created_at >= ?
      ...
      for (let i = 0; i < SEGMENT_COUNT; i++) {
        ...
        for (const call of segCalls) {
          const t = call.incident_type || 'Unknown';
          typeCounts[t] = (typeCounts[t] || 0) + 1;
        }
        const top_incident_types = Object.entries(typeCounts).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([type]) => type);

        let risk_score = 0;
        for (const call of segCalls) {
          risk_score += call.weapons_involved ? 8 : 0;
          risk_score += call.domestic_violence ? 5 : 0;
          risk_score += call.injuries_reported ? 4 : 0;
          risk_score += (call.priority === '1' || call.priority === 1) ? 3 : 0;
        }

        segments.push({
          start_lat: sLat, start_lng: sLng, end_lat: eLat, end_lng: eLng,
          call_count: segCalls.length, risk_score: Math.min(risk_score, 100), top_incident_types,
        });
      }
      ...
      segments.forEach((seg, i) => {
        if (seg.risk_score > maxRisk) { maxRisk = seg.risk_score; highest_risk_segment = i; }
      });

      return c.json({ segments, total_calls, highest_risk_segment });
    } catch {
      return c.json({ error: 'Failed to compute corridor analysis', code: 'CORRIDOR_ANALYSIS_ERROR' }, 500);
    }
  });

  // 4. GET /api/map/safety/unit-exposure/:callSign
  api.get('/unit-exposure/:callSign', requireRole(...OP_ROLES), async (c) => {
    try {
      const db = new D1Db(c.env.DB);
      const callSign = c.req.param('callSign') || '';
      if (!isValidCallSign(callSign)) {
        return c.json({ error: 'Invalid call sign format (alphanumeric, hyphens, underscores, 1-50 chars)', code: 'INVALID_CALL_SIGN' }, 400);
      }

      const unitRow = await db.prepare('SELECT id FROM units WHERE call_sign = ?').get(callSign) as any;
      const emptyResult = {
        call_sign: callSign, high_risk_minutes: 0, moderate_risk_minutes: 0, safe_minutes: 0,
        longest_exposure_minutes: 0, current_zone: 'safe', breadcrumb_count: 0,
      };
      if (!unitRow) return c.json(emptyResult);
      ...
      if (breadcrumbs.length === 0) return c.json(emptyResult);

      const dangerousAreas = await db.prepare(`
        SELECT latitude, longitude, weapons_involved, domestic_violence, injuries_reported
        FROM calls_for_service
        WHERE ((weapons_involved IS NOT NULL AND weapons_involved != '' AND weapons_involved != '0' AND weapons_involved != 'None') OR domestic_violence = 1 OR injuries_reported = 1)
          AND latitude IS NOT NULL AND longitude IS NOT NULL
      ...
      for (let i = 0; i < breadcrumbs.length; i++) {
        ...
        if (i < breadcrumbs.length - 1) {
          ...
          deltaMs = Math.min(next - curr, 300_000);
        }
        ...
        if (zone === 'high') { highRiskMs += deltaMs; currentExposureMs += deltaMs; }
        else if (zone === 'moderate') { moderateRiskMs += deltaMs; currentExposureMs += deltaMs; }
        else { safeMs += deltaMs; longestExposureMs = Math.max(longestExposureMs, currentExposureMs); currentExposureMs = 0; }

        if (i === breadcrumbs.length - 1) currentZone = zone;
      }
      ...
      return c.json({ error: 'Failed to compute unit exposure', code: 'UNIT_EXPOSURE_ERROR' }, 500);
    }
  });

  // 5. GET /api/map/safety/perimeter-check/:lat/:lng
  api.get('/perimeter-check/:lat/:lng', requireRole(...OP_ROLES), async (c) => {
    try {
      const db = new D1Db(c.env.DB);
      const lat = parseFloat(c.req.param('lat') || '');
      const lng = parseFloat(c.req.param('lng') || '');
      ...
      const bb = boundingBox(lat, lng, 10_000);
      ...
      return c.json({ quadrants, coverage_gaps, total_units_nearby, recommended_staging });
    } catch {
      return c.json({ error: 'Failed to compute perimeter check', code: 'PERIMETER_CHECK_ERROR' }, 500);
    }
  });

  // 6. GET /api/map/safety/shift-risk-summary
  api.get('/shift-risk-summary', requireRole(...OP_ROLES), async (c) => {
    try {
      const db = new D1Db(c.env.DB);
      ...
      const stats = await db.prepare(`
        ...
        FROM calls_for_service WHERE created_at >= ${shiftStartExpr}
      `).get() as any;

      const warrantCount = await db.prepare("SELECT COUNT(*) as cnt FROM warrants WHERE status = 'active'").get() as any;

      const dangerousAreasShift = await db.prepare(`
        SELECT latitude, longitude FROM calls_for_service
      ...
      return c.json({ error: 'Failed to compute shift risk summary', code: 'SHIFT_RISK_ERROR' }, 500);
    }
  });

  // 7. POST /api/map/safety/safety-alert
  const VALID_ALERT_TYPES = [
    'shots_fired', 'officer_down', 'pursuit', 'hazmat', 'armed_subject',
    'barricaded', 'hostage', 'bomb_threat', 'active_shooter', 'missing_officer',
  ] as const;

  api.post('/safety-alert', requireRole(...CMD_ROLES), async (c) => {
    try {
      const db = new D1Db(c.env.DB);
      const body = await c.req.json();
      const { type, lat, lng, details, radius_m } = body;

      if (!type || !VALID_ALERT_TYPES.includes(type)) {
        return c.json({ error: `Invalid alert type. Must be one of: ${VALID_ALERT_TYPES.join(', ')}` }, 400);
      }
      if (lat == null || lng == null) {
        return c.json({ error: 'lat and lng are required', code: 'LAT_AND_LNG_ARE' }, 400);
      }
      const latNum = Number(lat);
      const lngNum = Number(lng);
      if (!Number.isFinite(latNum) || !Number.isFinite(lngNum) || latNum < -90 || latNum > 90 || lngNum < -180 || lngNum > 180) {
        return c.json({ error: 'lat must be between -90 and 90, lng between -180 and 180', code: 'LAT_MUST_BE_BETWEEN' }, 400);
      }

      const sanitizedDetails = typeof details === 'string' ? details.replace(/<[^>]*>/g, '').substring(0, 500) : '';
      ...
      const alertData = {
        alert_id: Number(result.meta.last_row_id),
        type, lat: latNum, lng: lngNum,
        details: sanitizedDetails, radius_m: safeRadius,
        issued_by: c.get('user')?.username || 'Unknown',
        created_at: now,
      };

      await auditLog(db, c, 'safety_alert_broadcast', 'safety_alert', Number(result.meta.last_row_id),
        `Safety alert: ${type} at ${latNum.toFixed(4)},${lngNum.toFixed(4)}`);

      // WebSocket broadcast not available in Workers runtime
      return c.json({ success: true, alert_id: Number(result.meta.last_row_id), alert: alertData });
    } catch {
      return c.json({ error: 'Failed to issue safety alert', code: 'SAFETY_ALERT_ERROR' }, 500);
    }
  });

  // DELETE /api/map/safety/safety-alert/:id
  api.delete('/safety-alert/:id', requireRole('admin'), async (c) => {
    try {
      const db = new D1Db(c.env.DB);
      const id = parseInt(c.req.param('id') || '', 10);
      if (isNaN(id)) return c.json({ error: 'Invalid alert ID', code: 'INVALID_ALERT_ID' }, 400);

      const result = await db.prepare('DELETE FROM activity_log WHERE id = ? AND entity_type = ?').run(id, 'safety_alert');
      if (result.meta.changes === 0) return c.json({ error: 'Safety alert not found', code: 'SAFETY_ALERT_NOT_FOUND' }, 404);

      await auditLog(db, c, 'DELETE', 'safety_alert', id, `Deleted safety alert #${id}`);
      return c.json({ success: true });
    } catch {
      return c.json({ error: 'Failed to delete safety alert', code: 'DELETE_SAFETY_ALERT_ERROR' }, 500);
    }
  });

  // 8. GET /api/map/safety/repeat-offender-map
  api.get('/repeat-offender-map', requireRole(...OP_ROLES), async (c) => {
    try {
      const db = new D1Db(c.env.DB);
      let offenders: any[] = [];

      try {
        offenders = await db.prepare(`
          SELECT p.id as person_id, p.first_name, p.last_name, p.caution_flags,
            COUNT(cp.call_id) as call_count, MAX(c.created_at) as last_incident_date,
            c.latitude as last_lat, c.longitude as last_lng
          FROM persons p
          JOIN call_persons cp ON cp.person_id = p.id
          JOIN calls_for_service c ON c.id = cp.call_id
          WHERE c.latitude IS NOT NULL AND c.longitude IS NOT NULL
          GROUP BY p.id HAVING call_count >= 3
          ORDER BY call_count DESC LIMIT 50
        `).all();
      } catch {
        offenders = await db.prepare(`
          SELECT id as person_id, first_name, last_name, caution_flags,
            0 as call_count, NULL as last_incident_date, NULL as last_lat, NULL as last_lng
          FROM persons WHERE caution_flags IS NOT NULL AND caution_flags != ''
      ...
      return c.json({ offenders: result });
    } catch {
      return c.json({ error: 'Failed to query repeat offenders', code: 'REPEAT_OFFENDER_ERROR' }, 500);
    }
  });

  // 9. GET /api/map/safety/lighting-conditions
  api.get('/lighting-conditions', requireRole(...OP_ROLES), async (c) => {
    try {
      const now = new Date();
      const mt = new Date(now.toLocaleString('en-US', { timeZone: 'America/Denver' }));
      ...
      if (nowMs >= sunrise.getTime() && nowMs < sunset.getTime()) {
        ...
        if (nowMs > sunset.getTime()) tomorrowSunrise.setDate(tomorrowSunrise.getDate() + 1);
      ...
      return c.json({
        condition, minutes_to_transition, next_transition, tactical_note,
        sunrise: formatTime(sunrise), sunset: formatTime(sunset),
      });
    } catch {
      return c.json({ error: 'Failed to compute lighting conditions', code: 'LIGHTING_ERROR' }, 500);
    }
  });

  // 10. GET /api/map/safety/coverage-gaps
  api.get('/coverage-gaps', requireRole(...OP_ROLES), async (c) => {
    try {
      const db = new D1Db(c.env.DB);
      ...
      for (let lat = LAT_MIN; lat < LAT_MAX; lat += CELL_SIZE) {
        for (let lng = LNG_MIN; lng < LNG_MAX; lng += CELL_SIZE) {
          ...
          for (const unit of units) {
            ...
            if (dist < nearestDist) { nearestDist = dist; nearestUnit = unit.call_sign; }
          }
      ...
      return c.json({ error: 'Failed to compute coverage gaps', code: 'COVERAGE_GAPS_ERROR' }, 500);
    }
  });

  app.route('/api/map/safety', api);
}
