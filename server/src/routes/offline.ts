...
// ─── POST /sync/push-gps ─────────────────────────────────────
// Batch push GPS breadcrumbs from local storage
router.post('/sync/push-gps', (req: Request, res: Response) => {
  try {
    const { points } = req.body;
    const db = getDb();

    if (!Array.isArray(points) || points.length === 0) {
      res.status(400).json({ error: 'No GPS points to push' });
      return;
    }

    const stmt = db.prepare(`
      INSERT INTO gps_breadcrumbs (unit_id, officer_id, call_sign, latitude, longitude,
        accuracy, heading, speed, unit_status, officer_name, badge_number,
    ...
    const tx = db.transaction(() => {
      for (const p of points) {
        stmt.run(
          p.unit_id, p.officer_id, p.call_sign, p.latitude, p.longitude,
          p.accuracy, p.heading, p.speed, p.unit_status,
          p.officer_name, p.badge_number,
          p.current_call_id, p.current_call_number, p.current_call_type,
          p.recorded_at
        );
      }
    });
    ...
    res.json({ pushed: points.length });
  } catch (error: any) {
    console.error('[OFFLINE] GPS push error:', error.message);
    res.status(500).json({ error: 'Failed to push GPS data' });
  }
});
...
router.get('/secrets', requireRole('admin'), (req: Request, res: Response) => {
  try {
    ...
    const secrets = db.prepare(`
      SELECT ops.user_id, ops.secret, ops.created_at, ops.rotated_at,
...
// ─── POST /secrets/generate-pin ──────────────────────────────
// Server-side PIN generation — mirrors the desktop pinManager.js
// HMAC-SHA256 algorithm so admins can generate PINs from the
// web browser without needing the Electron app.
router.post('/secrets/generate-pin', requireRole('admin'), (req: Request, res: Response) => {
  try {
    const { userId } = req.body;
    const db = getDb();
    ...
    // Get the employee's secret
    const userRow = db.prepare(`
      SELECT ops.secret, u.username, u.full_name
      FROM offline_pin_secrets ops
      JOIN users u ON u.id = ops.user_id
      WHERE ops.user_id = ?
    `).get(userId) as { secret: string; username: string; full_name: string } | undefined;

    if (!userRow) {
      res.status(404).json({ error: 'No offline secret for this user — generate one first' });
      return;
    }

    // Get the admin's secret (requesting admin)
    const adminRow = db.prepare(
      'SELECT secret FROM offline_pin_secrets WHERE user_id = ?'
    ).get(req.user!.userId) as { secret: string } | undefined;

    if (!adminRow) {
      res.status(400).json({ error: 'Admin has no offline secret — generate one for yourself first' });
      return;
    }

    // ── Compute PIN (mirrors desktop/pinManager.js exactly) ──
    // Window start = midnight Mountain Time today
    const now = new Date();
    ...
    const windowStart = `${y}-${m}-${d}T00:00:00`;

    const payload = `${userRow.secret}:${windowStart}:${adminRow.secret}`;
    const hmac = crypto.createHmac('sha256', userRow.secret).update(payload).digest('hex');
    const num = parseInt(hmac.substring(0, 8), 16) % 1000000;
    const pin = String(num).padStart(6, '0');

    // Compute expiry (end of day, or next day if past 6pm)
    const expiry = new Date(mt);
    ...
    console.error('[OFFLINE] Generate PIN error:', error.message);
