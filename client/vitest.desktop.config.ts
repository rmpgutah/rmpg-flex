...
export default defineConfig({
  ...
  test: {
    ...
    testTimeout: 20_000,
    // maxWorkers:2 caps concurrent jsdom environments so they don't all compete
    ...
    // ⚠️ Vitest 4 removed `poolOptions` — the previous
    // `poolOptions: { forks: { maxForks: 2 } }` was silently IGNORED (vitest
    // printed a DEPRECATED notice and ran at full parallelism), which is why
    // the 2026-09-04/05 CI notes saw no effect from "capping" forks.
    pool: 'forks',
    maxWorkers: 2,
  },
});
