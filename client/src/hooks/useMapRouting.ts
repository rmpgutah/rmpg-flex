...
import { fetchMapboxRoute, hasMapboxDirections } from '../utils/mapboxRouting';
    ...
    async (
      ...
      try {
        ...
        if (hasMapboxDirections()) {
          const route = await fetchMapboxRoute(origin, destination);
          if (route) {
            ...
            polylineRef.current.setPath(route.geometry);
