  ...
  useEffect(() => {
    ...
    unsubs.push(
      subscribe('safety:broadcast', (msg) => {
        ...
        const text = `Safety alert. ${toDisplayLabel(alertType).toUpperCase()}${unit ? ` reported by ${unit}` : ''} at ${location}.`;
    ...
    unsubs.push(
      subscribe('security:updated', (msg) => {
        ...
        if (['device_removed', 'sessions_terminated', 'ip_blocked'].includes(action)) {
          speak(`Security alert. ${toDisplayLabel(action).toUpperCase()}.`, 'moderate');
