...
export interface PatrolBeatPlannerModalProps {
  onClose(): void;
...
export default function PatrolBeatPlannerModal({ onClose, onSolutionReady }: PatrolBeatPlannerModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div
        className="bg-surface-base border border-rmpg-600 p-4 w-full max-w-md mx-4 max-h-[90vh] flex flex-col gap-3"
        style={{ borderRadius: 2 }}
      >
        {/* Header */}
        <div className="flex items-center justify-between">
          <span className="text-sm font-semibold text-rmpg-100">Patrol Beat Planner</span>
          <button type="button" onClick={onClose} className="text-rmpg-400 hover:text-rmpg-100 text-xs">
            ✕
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="flex flex-col gap-0.5">
            <label className="text-[10px]" style={{ color: 'var(--field-label-color)' }}>Shift Start (UTC)</label>
            <input
              type="datetime-local"
              value={shiftStart}
              step={60}
          ...
          <div className="flex flex-col gap-0.5">
            <label className="text-[10px]" style={{ color: 'var(--field-label-color)' }}>Shift End (UTC)</label>
            <input
              type="datetime-local"
              value={shiftEnd}
              step={60}
