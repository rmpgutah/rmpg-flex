 ...
 *
 * Works in both Electron (local HMAC) and web browser (server API).
 */
export default function PinGeneratorModal({ isOpen, onClose, users }: PinGeneratorModalProps) {
  const { generatePin, isElectron } = useOfflineMode();
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      if (isElectron) {
        // Desktop app — compute PIN locally via Electron IPC
        result = await generatePin(selectedUserId);
      } else {
        // Web browser — call server-side PIN generation
        result = await apiFetch<any>('/offline/secrets/generate-pin', {
          method: 'POST',
  ...
  }, [selectedUserId, generatePin, isElectron]);
