...
export default function ForcePasswordChangeModal() {
  ...
  useEffect(() => {
    apiFetch<any>('/auth/password-policy')
      .then(data => setPolicyRules(Array.isArray(data?.policy) ? data.policy : []))
      .catch(() => {});
  }, []);

  // Live policy validation display
      ...
      >
        ...
        <div className="text-center space-y-2">
          <div className="flex items-center justify-center gap-2">
            <Shield style={{ width: 20, height: 20, color: '#bc1010' }} />
              ...
              {rules.map((rule, i) => (
                <div key={i} className="flex items-center gap-2 text-[10px]">
                  {rule.met ? (
                    ...
                    <AlertCircle style={{ width: 10, height: 10, color: '#707070', flexShrink: 0 }} />
                  )}
