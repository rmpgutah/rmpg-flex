...
describe('nearestNeighborOrder', () => {
  ...
  it('duration is drive time PLUS per-stop dwell time (12 min/individual stop)', () => {
    // Duration includes a per-stop dwell (knock/serve/paperwork) on top of
    // drive time. Individual stops default to 12 minutes each.
    const stops = [stop(1, 40.5, -111.9), stop(2, 40.9, -111.9)];
    const r = nearestNeighborOrder(stops, null);
    expect(r.totalDurationMinutes).toBeCloseTo(estimateDriveMinutes(r.totalDistanceMiles) + 2 * 12, 6);
  });
