  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black flex flex-col">
      ...
      <div className="flex items-center justify-center gap-2 px-4 py-3 bg-[#0a0a0a] border-t border-[#222222] flex-shrink-0 flex-wrap">
        {torchAvailable && (
          <button type="button" onClick={toggleTorch} className={`flex items-center gap-1.5 px-3 py-2 rounded-sm text-[10px] font-bold border transition-colors ${torchOn ? 'bg-[#d4a017] text-black border-[#d4a017]' : 'bg-[#141414] text-[#c0ccdd] border-[#2e2e2e] hover:text-rmpg-100'}`}>
        ...
        {step === 'front' ? (
          <>
            ...
            <button type="button" onClick={skipFront} className="px-3 py-2 bg-[#141414] border border-[#2e2e2e] rounded-sm text-[10px] font-bold text-[#8899aa] hover:text-rmpg-100">Skip</button>
          </>
        ) : (
          <>
            {frontPreview && (
              <button type="button" onClick={retakeFront} className="flex items-center gap-1.5 px-3 py-2 bg-[#141414] border border-[#2e2e2e] rounded-sm text-[10px] font-bold text-[#c0ccdd] hover:text-rmpg-100">
                <RotateCcw size={12} /> Retake Front
              </button>
            )}
            <button type="button" onClick={captureBackNoBarcode} className="flex items-center gap-1.5 px-3 py-2 bg-[#141414] border border-[#2e2e2e] rounded-sm text-[10px] font-bold text-[#c0ccdd] hover:text-rmpg-100">
              <CreditCard size={13} /> Capture Back (no barcode)
            </button>
          </>
        )}

        <button type="button" onClick={() => { stopStream(); onUploadInstead(); }} className="flex items-center gap-1.5 px-3 py-2 bg-[#141414] border border-[#2e2e2e] rounded-sm text-[10px] font-bold text-[#c0ccdd] hover:text-rmpg-100">
          <Upload size={13} /> Upload
        </button>
        <button type="button" onClick={() => { stopStream(); onClose(); }} className="px-3 py-2 bg-[#141414] border border-[#2e2e2e] rounded-sm text-[10px] font-bold text-[#8899aa] hover:text-rmpg-100">Cancel</button>
      </div>
    </div>
  );
}
...
function StepDot({ active, done, label }: { active: boolean; done: boolean; label: string }) {
  return (
    <div className="flex items-center gap-1">
      <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[7px] font-bold ${done ? 'bg-green-600 text-rmpg-100' : active ? 'bg-[#d4a017] text-black' : 'bg-[#1a1a1a] text-[#556677] border border-[#2e2e2e]'}`}>
