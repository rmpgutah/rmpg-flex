...
export default function VoiceChannelIndicator() {
    ...
    return (
      <div className="fixed right-4 z-[9999]" style={{ bottom: 'max(2rem, env(safe-area-inset-bottom, 2rem))' }}>
        <button
          ...
          className="flex items-center gap-1.5 px-2.5 py-1.5 bg-[#1b2128] border border-[#2a3a4e] rounded text-gray-400 hover:border-[#888888] hover:text-white transition-colors text-xs font-mono"
  ...
  return (
    <div className="fixed right-4 z-[9999] flex flex-col items-end gap-2 max-w-xs" style={{ bottom: 'max(2rem, env(safe-area-inset-bottom, 2rem))' }}>
      ...
      {/* Transcript */}
      {transcript && (
        <div className="bg-[#1b2128] border border-[#2a3a4e] rounded px-3 py-2 text-xs font-mono text-green-400 max-w-xs break-words">
          {transcript}
        </div>
      )}
      ...
      {lastCommand && (
        <div
          className={`bg-[#1b2128] rounded px-3 py-2 text-xs font-mono max-w-xs break-words border ${
      ...
      {/* Error */}
      {error && (
        <div className="bg-[#1b2128] border border-red-600 rounded px-3 py-2 text-xs font-mono text-red-400 max-w-xs break-words">
          {error}
        </div>
      )}
    </div>
  ...
  return (
    <div className="flex items-center gap-2">
      {state === 'listening' && (
        <button
          onClick={onClickMic}
          className="flex items-center gap-1.5 px-2.5 py-1.5 bg-[#1b2128] border border-green-600 rounded text-green-400 hover:text-white transition-colors text-xs font-mono"
