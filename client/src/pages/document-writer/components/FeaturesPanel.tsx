...
export default function FeaturesPanel({ editor, onClose, caseUrl }: Props) {
  ...
  return (
    <div className="w-[360px] flex-shrink-0 bg-[#0a0a0a] border border-[#222] rounded-[2px] flex flex-col text-rmpg-200 text-xs">
      ...
      <div className="flex flex-wrap gap-1 px-2 py-2 border-b border-[#1a1a1a]">
        {tabs.map(t => {
          const active = tab === t.id;
          return (
            <button key={t.id} type="button"
              onClick={() => { setTab(t.id); setQ(''); }}
              className={`flex items-center gap-1 px-2 py-1 text-[10px] rounded-[2px] border transition-colors ${active ? 'bg-[#d4a017]/15 border-[#d4a017]/40 text-[#d4a017]' : 'bg-[#0d0d0d] border-[#222] text-rmpg-400 hover:text-rmpg-200'}`}>
      ...
      {tab !== 'tools' && tab !== 'userTemplates' && (
        <div className="px-3 py-2 border-b border-[#1a1a1a] relative">
      ...
      <div className="flex-1 overflow-auto p-2 space-y-3">
          ...
          <div className="space-y-1">
            ...
            {statuteHits.map((h) => (
              <button key={h.code} type="button" onClick={() => insertStatute(editor, h)}
                className="w-full text-left p-2 bg-[#0d0d0d] border border-[#222] rounded-[2px] hover:border-[#d4a017]/40">
        ...
        {tab === 'tools' && (
          <div className="space-y-3">
            ...
            {readout && <div className="text-[10px] text-rmpg-300 bg-[#0d0d0d] border border-[#222] p-2 rounded-[2px]">{readout}</div>}
          </div>
        )}
        ...
        {tab === 'userTemplates' && (
          <div className="space-y-1">
            ...
            {userTpls.map(t => (
              <div key={t.id} className="flex items-center gap-1 p-2 bg-[#0d0d0d] border border-[#222] rounded-[2px]">
...
function Btn({ icon, label, onClick, active }: { icon: React.ReactNode; label: string; onClick: () => void; active?: boolean }) {
  return (
    <button type="button" onClick={onClick}
      className={`flex items-center gap-1.5 px-2 py-1.5 text-[10px] rounded-[2px] border transition-colors ${active ? 'bg-[#d4a017]/15 border-[#d4a017]/40 text-[#d4a017]' : 'bg-[#0d0d0d] border-[#222] text-rmpg-300 hover:border-[#d4a017]/40 hover:text-rmpg-100'}`}>
