...
export default function AccreditationPage() {
  ...
  if (loading) return <div className="p-6 text-rmpg-400">Loading accreditation data...</div>;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={() => setFormOpen(false)}>
          <div className="bg-surface-raised border border-rmpg-700 p-6 max-w-lg w-full" style={{ borderRadius: 2 }} onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label htmlFor="ff-accreditationpage-0" className="text-[9px] text-rmpg-400 uppercase font-bold">Standard # *</label><input id="ff-accreditationpage-0" value={formData.standard_number} onChange={e => setFormData({...formData, standard_number: e.target.value})} className="input-dark w-full mt-1 text-xs" /></div>
              <div><label htmlFor="ff-accreditationpage-1" className="text-[9px] text-rmpg-400 uppercase font-bold">Name *</label><input id="ff-accreditationpage-1" value={formData.standard_name} onChange={e => setFormData({...formData, standard_name: e.target.value})} className="input-dark w-full mt-1 text-xs" /></div>
              <div><label htmlFor="ff-accreditationpage-2" className="text-[9px] text-rmpg-400 uppercase font-bold">Category</label><input id="ff-accreditationpage-2" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})} className="input-dark w-full mt-1 text-xs" /></div>
              <div><label htmlFor="ff-accreditationpage-3" className="text-[9px] text-rmpg-400 uppercase font-bold">Status</label><select id="ff-accreditationpage-3" value={formData.compliance_status} onChange={e => setFormData({...formData, compliance_status: e.target.value})} className="input-dark w-full mt-1 text-xs"><option value="pending">Pending</option><option value="in_progress">In Progress</option><option value="compliant">Compliant</option><option value="non_compliant">Non-Compliant</option></select></div>
              <div><label htmlFor="ff-accreditationpage-4" className="text-[9px] text-rmpg-400 uppercase font-bold">Last Reviewed</label><input id="ff-accreditationpage-4" type="date" value={formData.last_reviewed} onChange={e => setFormData({...formData, last_reviewed: e.target.value})} className="input-dark w-full mt-1 text-xs" /></div>
              <div><label htmlFor="ff-accreditationpage-5" className="text-[9px] text-rmpg-400 uppercase font-bold">Next Review</label><input id="ff-accreditationpage-5" type="date" value={formData.next_review} onChange={e => setFormData({...formData, next_review: e.target.value})} className="input-dark w-full mt-1 text-xs" /></div>
            </div>
            <div className="mt-3"><label htmlFor="ff-accreditationpage-6" className="text-[9px] text-rmpg-400 uppercase font-bold">Description</label><textarea id="ff-accreditationpage-6" value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} className="input-dark w-full mt-1 text-xs" rows={2} /></div>
            <div className="mt-3"><label htmlFor="ff-accreditationpage-7" className="text-[9px] text-rmpg-400 uppercase font-bold">Notes</label><textarea id="ff-accreditationpage-7" value={formData.notes} onChange={e => setFormData({...formData, notes: e.target.value})} className="input-dark w-full mt-1 text-xs" rows={2} /></div>
            <div className="flex justify-end gap-3 mt-4">
              <button onClick={() => setFormOpen(false)} className="toolbar-btn px-4" style={{ height: 28 }}>Cancel</button>
              <button onClick={handleSave} disabled={formSubmitting || !formData.standard_number || !formData.standard_name} className="toolbar-btn toolbar-btn-primary px-4" style={{ height: 28 }}>{formSubmitting ? 'Saving...' : 'Save'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {deleteId !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={() => setDeleteId(null)}>
          <div className="bg-surface-raised border border-red-800 p-6 max-w-sm w-full" style={{ borderRadius: 2 }} onClick={e => e.stopPropagation()}>
            ...
            <p className="text-xs text-rmpg-400 mb-4">This permanently removes this accreditation standard. This cannot be undone.</p>
