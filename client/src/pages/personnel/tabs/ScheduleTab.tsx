...
export default function ScheduleTab({ officers, schedules, weekMonday, onWeekChange, onAddSchedule }: Props) {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 space-y-3">
      {/* Section Header with Add Schedule button */}
          ...
          <div className="grid grid-cols-8 min-w-[700px]">
            ...
            {weekDates.map((d, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`p-2 text-center border-b border-rmpg-700 bg-gradient-to-b from-rmpg-800 to-surface-sunken ${
                    isToday ? 'schedule-today' : ''
                  }`}
                >
              ...
              <React.Fragment key={officer.id}>
                ...
                {weekDates.map((d, dayIdx) => {
                  ...
                  return (
                    <div
                      key={dayIdx}
                      className={`p-1 border-b border-rmpg-700/50 min-h-[44px] ${
                        isToday ? 'schedule-today' : ''
                      }`}
                    >
                      {cellSchedules.map((s) => {
