...
const STATUS_DOT: Record<string, string> = {
  active: 'bg-emerald-500', pending: 'bg-[#888888]', disabled: 'bg-red-600',
};
...
export default function JailRecordsPage() {
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      {/* Manual ingest */}
      <div className="bg-[#141414] border border-[#222222] p-3 space-y-2">
        ...
        <div className="flex gap-2">
          ...
          <select value={format} onChange={(e) => setFormat(e.target.value as 'lines' | 'csv')}
            className="bg-[#050505] border border-[#222222] px-2 py-1 text-[11px] text-gray-200">
        ...
        <textarea value={text} onChange={(e) => setText(e.target.value)} rows={4}
          ...
          className="w-full bg-[#050505] border border-[#222222] px-2 py-1 text-[11px] text-gray-200 focus:border-[#d4a017] outline-none font-mono" />
        <button onClick={ingest} disabled={busy || !text.trim()}
          className="w-full py-1.5 text-[11px] font-semibold border border-[#d4a017] text-[#d4a017] hover:bg-[#1a1a1a] disabled:opacity-40">
      ...
      <div className="bg-[#141414] border border-[#222222]">
        <div className="px-2 py-[3px] text-[9px] font-semibold text-[#d4a017] border-b border-[#1a1a1a]">RECENT BOOKINGS</div>
        {bookings.length === 0 && <div className="p-2 text-[11px] text-[#888888]">None yet — ingest a roster above.</div>}
        {bookings.map((b) => (
          <div key={b.id} className="px-2 py-[2px] text-[11px] flex items-center gap-2 border-b border-[#1a1a1a] last:border-b-0">
            <span className="text-gray-200 w-44 shrink-0 truncate">{b.full_name}</span>
            <span className="text-[#888888] flex-1 truncate">{b.charges || ''}</span>
            <span className="text-[#888888]">{b.county || ''}</span>
            <span className="text-[#888888]">{b.booking_date ? String(b.booking_date).slice(0, 10) : ''}</span>
            {b.person_id
              ? <Link to={`/intel/person/${b.person_id}`} className="text-[9px] text-[#d4a017] border border-[#222222] px-1">KNOWN →</Link>
              : <span className="text-[9px] text-[#888888] w-12 text-right">new</span>}
          </div>
        ))}
      </div>
      ...
      <div className="bg-[#141414] border border-[#222222]">
        ...
        {[...active, ...pending].map((s) => (
          <div key={s.source_key} className="px-2 py-[2px] text-[11px] flex items-center gap-2 border-b border-[#1a1a1a] last:border-b-0">
            <span className={`w-2 h-2 rounded-full shrink-0 ${STATUS_DOT[s.status] || 'bg-[#888888]'}`} />
            <span className="text-gray-200 flex-1 truncate">{s.display_name}</span>
            <span className="text-[#888888] text-[9px]">{s.kind}</span>
            <span className="text-[#888888] text-[9px]">{s.row_count || 0} rec</span>
            <span className="text-[#888888] text-[9px] w-28 truncate text-right">{s.last_status || s.status}</span>
          </div>
        ))}
      </div>
