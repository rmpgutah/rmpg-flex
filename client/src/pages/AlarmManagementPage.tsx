...
export default function AlarmManagementPage() {
  ...
  const columns = [
    ...
    { key: 'status', label: 'Status', render: (r: AlarmAccount) => <span className={`badge ${r.status === 'active' ? 'badge-available' : 'badge-p4'}`}>{formatEnumValue(r.status)}</span> },
  ...
  if (loading) return <div className="p-6 text-rmpg-400">Loading alarm data...</div>;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={() => setFormOpen(false)}>
          <div className="bg-surface-raised border border-rmpg-700 p-6 max-w-lg w-full" style={{ borderRadius: 2 }} onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label htmlFor="ff-alarmmanagementpage-0" className="text-[9px] text-rmpg-400 uppercase font-bold">Account # *</label><input id="ff-alarmmanagementpage-0" value={formData.account_number} onChange={e => setFormData({...formData, account_number: e.target.value})} className="input-dark w-full mt-1 text-xs" /></div>
              <div><label htmlFor="ff-alarmmanagementpage-1" className="text-[9px] text-rmpg-400 uppercase font-bold">Name *</label><input id="ff-alarmmanagementpage-1" value={formData.account_name} onChange={e => setFormData({...formData, account_name: e.target.value})} className="input-dark w-full mt-1 text-xs" /></div>
              <div className="col-span-2"><label htmlFor="ff-alarmmanagementpage-2" className="text-[9px] text-rmpg-400 uppercase font-bold">Address *</label><input id="ff-alarmmanagementpage-2" value={formData.address} onChange={e => setFormData({...formData, address: e.target.value})} className="input-dark w-full mt-1 text-xs" /></div>
              <div><label htmlFor="ff-alarmmanagementpage-3" className="text-[9px] text-rmpg-400 uppercase font-bold">Contact</label><input id="ff-alarmmanagementpage-3" value={formData.contact_name} onChange={e => setFormData({...formData, contact_name: e.target.value})} className="input-dark w-full mt-1 text-xs" /></div>
              <div><label htmlFor="ff-alarmmanagementpage-4" className="text-[9px] text-rmpg-400 uppercase font-bold">Phone</label><input id="ff-alarmmanagementpage-4" value={formData.contact_phone} onChange={e => setFormData({...formData, contact_phone: e.target.value})} className="input-dark w-full mt-1 text-xs" /></div>
              <div><label htmlFor="ff-alarmmanagementpage-5" className="text-[9px] text-rmpg-400 uppercase font-bold">Permit #</label><input id="ff-alarmmanagementpage-5" value={formData.permit_number} onChange={e => setFormData({...formData, permit_number: e.target.value})} className="input-dark w-full mt-1 text-xs" /></div>
              <div><label htmlFor="ff-alarmmanagementpage-6" className="text-[9px] text-rmpg-400 uppercase font-bold">Permit Status</label><select id="ff-alarmmanagementpage-6" value={formData.permit_status} onChange={e => setFormData({...formData, permit_status: e.target.value})} className="input-dark w-full mt-1 text-xs"><option value="active">Active</option><option value="expired">Expired</option><option value="suspended">Suspended</option><option value="pending">Pending</option></select></div>
              <div><label htmlFor="ff-alarmmanagementpage-7" className="text-[9px] text-rmpg-400 uppercase font-bold">Permit Expiry</label><input id="ff-alarmmanagementpage-7" type="date" value={formData.permit_expiry} onChange={e => setFormData({...formData, permit_expiry: e.target.value})} className="input-dark w-full mt-1 text-xs" /></div>
              <div><label htmlFor="ff-alarmmanagementpage-8" className="text-[9px] text-rmpg-400 uppercase font-bold">Alarm Type</label><select id="ff-alarmmanagementpage-8" value={formData.alarm_type} onChange={e => setFormData({...formData, alarm_type: e.target.value})} className="input-dark w-full mt-1 text-xs"><option value="burglary">Burglary</option><option value="robbery">Robbery</option><option value="panic">Panic</option><option value="fire">Fire</option><option value="medical">Medical</option><option value="other">Other</option></select></div>
              <div><label htmlFor="ff-alarmmanagementpage-9" className="text-[9px] text-rmpg-400 uppercase font-bold">False Alarms</label><input id="ff-alarmmanagementpage-9" type="number" value={formData.false_alarm_count} onChange={e => setFormData({...formData, false_alarm_count: parseInt(e.target.value) || 0})} className="input-dark w-full mt-1 text-xs" /></div>
              <div><label htmlFor="ff-alarmmanagementpage-10" className="text-[9px] text-rmpg-400 uppercase font-bold">Status</label><select id="ff-alarmmanagementpage-10" value={formData.status} onChange={e => setFormData({...formData, status: e.target.value})} className="input-dark w-full mt-1 text-xs"><option value="active">Active</option><option value="inactive">Inactive</option><option value="no_response">No Response</option></select></div>
            </div>
            <div className="mt-3"><label htmlFor="ff-alarmmanagementpage-11" className="text-[9px] text-rmpg-400 uppercase font-bold">Notes</label><textarea id="ff-alarmmanagementpage-11" value={formData.notes} onChange={e => setFormData({...formData, notes: e.target.value})} className="input-dark w-full mt-1 text-xs" rows={3} /></div>
            <div className="flex justify-end gap-3 mt-4">
              <button onClick={() => setFormOpen(false)} className="toolbar-btn px-4" style={{ height: 28 }}>Cancel</button>
              <button onClick={handleSave} disabled={formSubmitting || !formData.account_number || !formData.account_name || !formData.address} className="toolbar-btn toolbar-btn-primary px-4" style={{ height: 28 }}>{formSubmitting ? 'Saving...' : 'Save'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {deleteId !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={() => setDeleteId(null)}>
          <div className="bg-surface-raised border border-red-800 p-6 max-w-sm w-full" style={{ borderRadius: 2 }} onClick={e => e.stopPropagation()}>
            ...
            <p className="text-xs text-rmpg-400 mb-4">This permanently removes this alarm account. This cannot be undone.</p>
