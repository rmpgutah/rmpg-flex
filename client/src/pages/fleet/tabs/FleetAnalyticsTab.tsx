...
export default function FleetAnalyticsTab({ analytics, loading }: Props) {
  ...
  useEffect(() => {
    apiFetch<any>('/api/fleet/fleet-cost-analytics').then((d: any) => d && setCostAnalytics(d)).catch(() => {});
    apiFetch<any>('/api/fleet/service-interval-alerts').then((d: any) => d?.alerts && setServiceAlerts(d.alerts)).catch(() => {});
    apiFetch<any>('/api/fleet/inspection-stats').then((d: any) => d && setInspectionStats(d)).catch(() => {});
    apiFetch<any>('/api/fleet/notifications').then((d: any) => d?.notifications && setNotifications(d.notifications)).catch(() => {});
    apiFetch<any>('/api/fleet/overdue-inspections').then((d: any) => d?.alerts && setOverdueInspections(d.alerts)).catch(() => {});
  }, []);
