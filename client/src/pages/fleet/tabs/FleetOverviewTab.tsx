...
export default function FleetOverviewTab({ detail, maintenance, onEditMaintenance, onDeleteMaintenance }: Props) {
  ...
  useEffect(() => {
    if (!detail?.id) return;
    apiFetch(`/api/fleet/${detail.id}/fuel-efficiency`).then(r => r.ok ? r.json() : null).then(d => d && setFuelEfficiency(d));
    apiFetch(`/api/fleet/${detail.id}/maintenance-costs`).then(r => r.ok ? r.json() : null).then(d => d && setMaintenanceCosts(d));
    apiFetch(`/api/fleet/${detail.id}/mileage-history`).then(r => r.ok ? r.json() : null).then(d => Array.isArray(d) && setMileageHistory(d));
  }, [detail?.id]);
