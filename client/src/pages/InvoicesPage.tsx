...
export default function InvoicesPage() {
  ...
  const StatsBar = () => {
    ...
    return (
      <div className="flex items-center gap-4 px-3 py-1.5 bg-[#0c0f13] border border-[#2b313a] text-[10px]" style={{ borderRadius: '2px' }}>
        {items.map((it, i) => (
          <React.Fragment key={it.label}>
            {i > 0 && <div className="w-px h-3.5 bg-[#2b313a]" />}
      ...
      <div className="card-glass p-4 space-y-3">
        ...
        <div>
          ...
          <select
            ...
            className="w-full bg-[#0c0f13] border border-[#2b313a] rounded-sm px-2 py-1.5 text-xs text-white focus:border-brand-500 focus:ring-1 focus:ring-brand-500/30 focus:outline-none"
          >
        ...
        <div>
          ...
          <textarea
            ...
            rows={2}
            className="w-full bg-[#0c0f13] border border-[#2b313a] rounded-sm px-2 py-1.5 text-xs text-white focus:border-brand-500 focus:ring-1 focus:ring-brand-500/30 focus:outline-none resize-none"
          />
        </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-[#2b313a] scrollbar-track-transparent p-4 space-y-4">
        ...
        <div className="card-glass p-3">
          ...
          <div className="space-y-1 text-xs">
            ...
            <div className="border-t border-[#2b313a] my-1" />
        ...
        <div className="card-glass p-3">
          ...
          {/* Add line item form */}
          {showLineItemForm && (
            <div className="mb-3 p-2 bg-[#0c0f13] border border-[#2b313a] rounded-sm space-y-2">
              <select
                value={lineItemForm.line_type}
                onChange={e => setLineItemForm(f => ({ ...f, line_type: e.target.value }))}
                className="w-full bg-[#161b21] border border-[#2b313a] rounded-sm px-2 py-1 text-xs text-white"
              >
              ...
              <div className="grid grid-cols-2 gap-2">
                <input
                  ...
                  className="bg-[#161b21] border border-[#2b313a] rounded-sm px-2 py-1 text-xs text-white"
                />
                <input
                  type="number"
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead className="sticky top-0 z-10 bg-[#0c0f13]">
                  <tr className="text-[9px] uppercase tracking-wider text-rmpg-500 border-b border-[#2b313a]">
                ...
                <tbody>
                  {inv.line_items.map(item => (
                    <tr key={item.id} className="border-b border-[#2b313a]/50 hover:bg-[#1b2128]/50 transition-colors">
          ...
          {inv.payments && inv.payments.length > 0 ? (
            <div className="space-y-1">
              {inv.payments.map(pay => (
                <div key={pay.id} className="flex items-center justify-between py-1 border-b border-[#2b313a]/50 text-xs">
                  <div className="flex items-center gap-2">
  ...
  const InvoiceRow = ({ inv }: { inv: Invoice }) => {
    ...
    return (
      <tr
        ...
        className={`cursor-pointer border-b border-[#2b313a]/40 transition-colors text-xs ${
          isSelected
      ...
      <div className="app-grid-bg h-full flex flex-col">
        {mode === 'detail' && selectedInvoice ? (
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            {/* Toolbar */}
            <div className="p-2 space-y-2 border-b border-[#2b313a]">
              <div className="flex items-center gap-2">
                <div className="flex-1 relative">
                  ...
                  <input
                    ...
                    className="w-full bg-[#0c0f13] border border-[#2b313a] rounded-sm pl-7 pr-2 py-1.5 text-xs text-white focus:border-brand-500 focus:ring-1 focus:ring-brand-500/30 focus:outline-none"
                  />
                </div>
                {canEdit && (
                  <button type="button"
              ...
              <div className="flex items-center gap-2">
                <select
                  ...
                  className="bg-[#0c0f13] border border-[#2b313a] rounded-sm px-2 py-1 text-xs text-white"
                >
            ...
            <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-[#2b313a] scrollbar-track-transparent">
              {loading ? (
                ...
                <div className="divide-y divide-[#2b313a]/40">
                  {invoices.map(inv => (
                    <div
                      key={inv.id}
                      onClick={() => selectInvoice(inv)}
                      className="p-2 hover:bg-[#1b2128]/60 cursor-pointer transition-colors"
                    >
                      <div className="flex items-center justify-between">
  ...
  return (
    <div className="app-grid-bg h-full flex flex-col">
      {/* Toolbar */}
      <div className="flex items-center justify-between gap-3 px-3 py-1.5 border-b border-[#2b313a] flex-shrink-0">
        <div className="flex items-center gap-2">
      ...
      {/* Filters bar */}
      <div className="flex items-center gap-2 px-3 py-1 border-b border-[#2b313a]/60 flex-shrink-0 bg-[#0c0f13]/50">
        ...
        <div className="relative flex-1 max-w-xs">
          ...
          <input
            ...
            value={searchQuery}
            onChange={e => handleSearchChange(e.target.value)}
            className="w-full bg-[#161b21] border border-[#2b313a] rounded-sm pl-6 pr-2 py-1 text-[11px] text-white focus:border-brand-500 focus:ring-1 focus:ring-brand-500/30 focus:outline-none"
          />
        </div>
        <select
          value={filterStatus}
          onChange={e => { setFilterStatus(e.target.value as any); setPage(1); }}
          className="bg-[#161b21] border border-[#2b313a] rounded-sm px-2 py-1 text-[11px] text-white focus:outline-none focus:border-brand-500 transition-colors"
        >
          {STATUSES.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}
        </select>
        <select
          ...
          className="bg-[#161b21] border border-[#2b313a] rounded-sm px-2 py-1 text-[11px] text-white focus:outline-none focus:border-brand-500 transition-colors max-w-[160px]"
        >
        ...
        <input
          ...
          placeholder="From"
          className="bg-[#161b21] border border-[#2b313a] rounded-sm px-2 py-1 text-[11px] text-white focus:outline-none"
        />
        <input
          type="date"
          value={dateTo}
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Left: invoice list */}
        <div className="flex flex-col w-[55%] border-r border-[#2b313a] overflow-hidden">
          <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-[#2b313a] scrollbar-track-transparent">
            {loading ? (
              <div className="flex items-center justify-center gap-2 h-32"><Loader2 size={20} className="animate-spin text-brand-400" /><span className="text-xs text-rmpg-400">Loading invoices...</span></div>
            ) : invoices.length === 0 ? (
          ...
          {/* Pagination */}
          {totalPages > 1 && (
            <div className="flex items-center justify-between px-3 py-1 border-t border-[#2b313a] text-[10px] text-rmpg-500 flex-shrink-0">
