...
export default function InvoicesPage() {
  ...
  const getStatusActions = (inv: InvoiceDetail) => {
    ...
    const s = inv.status;
    if (s === 'draft') {
      actions.push({ label: 'Mark Sent', status: 'sent', icon: Send, cls: 'bg-brand-600 hover:bg-brand-500' });
          ...
          {showLineItemForm && (
            <div className="mb-3 p-2 bg-surface-sunken border border-rmpg-700 rounded-sm space-y-2">
              <select
                ...
                className="w-full bg-surface-base border border-rmpg-700 rounded-sm px-2 py-1 text-xs text-white"
              >
      ...
      <div className="flex items-center gap-2 px-3 py-1 border-b border-rmpg-700/60 flex-shrink-0 bg-surface-sunken/50">
        ...
        <div className="relative flex-1 max-w-xs">
          ...
          <input
            ...
            value={searchQuery}
            onChange={e => handleSearchChange(e.target.value)}
            className="w-full bg-surface-base border border-rmpg-700 rounded-sm pl-6 pr-2 py-1 text-[11px] text-white focus:border-brand-500 focus:outline-none"
          />
        </div>
        <select
          value={filterStatus}
          onChange={e => { setFilterStatus(e.target.value as any); setPage(1); }}
          className="bg-surface-base border border-rmpg-700 rounded-sm px-2 py-1 text-[11px] text-white focus:outline-none"
        >
          {STATUSES.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}
        </select>
        <select
          ...
          className="bg-surface-base border border-rmpg-700 rounded-sm px-2 py-1 text-[11px] text-white focus:outline-none max-w-[160px]"
        >
