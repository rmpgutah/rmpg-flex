          ...
          <div key={b.id} role="listitem" tabIndex={0}
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-1">
                ...
                {isAdmin && <button type="button" onClick={e => { e.stopPropagation(); openEdit(b); }} className="max-md:hidden p-0.5 hover:bg-rmpg-700 text-rmpg-500 hover:text-brand-400"><Pencil className="w-3 h-3" /></button>}
                {isAdmin && <button type="button" onClick={e => { e.stopPropagation(); setDeleteTarget({ type: 'business', id: b.id, label: b.name }); }} className="max-md:hidden p-0.5 hover:bg-rmpg-700 text-rmpg-500 hover:text-red-400"><Trash2 className="w-3 h-3" /></button>}
              </div>
            </div>
          </div>
        ))}
