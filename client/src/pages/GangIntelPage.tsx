...
export default function GangIntelPage() {
  ...
  const memberColumns = [
    ...
    { key: 'status', label: 'Status', render: (r: GangMember) => <span className={`badge ${r.status === 'active' ? 'badge-p1' : 'badge-p4'}`}>{formatEnumValue(r.status)}</span> },
  ...
  if (loading) return <div className="p-6 text-rmpg-400">Loading gang intelligence...</div>;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={() => setFormOpen(false)}>
          <div className="bg-surface-raised border border-rmpg-700 p-6 max-w-lg w-full" style={{ borderRadius: 2 }} onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label htmlFor="ff-gangintelpage-0" className="text-[9px] text-rmpg-400 uppercase font-bold">Name *</label><input id="ff-gangintelpage-0" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="input-dark w-full mt-1 text-xs" /></div>
              <div><label htmlFor="ff-gangintelpage-1" className="text-[9px] text-rmpg-400 uppercase font-bold">Moniker</label><input id="ff-gangintelpage-1" value={formData.moniker} onChange={e => setFormData({...formData, moniker: e.target.value})} className="input-dark w-full mt-1 text-xs" /></div>
              <div><label htmlFor="ff-gangintelpage-2" className="text-[9px] text-rmpg-400 uppercase font-bold">Gang</label><input id="ff-gangintelpage-2" value={formData.gang_name} onChange={e => setFormData({...formData, gang_name: e.target.value})} className="input-dark w-full mt-1 text-xs" /></div>
              <div><label htmlFor="ff-gangintelpage-3" className="text-[9px] text-rmpg-400 uppercase font-bold">Status</label><select id="ff-gangintelpage-3" value={formData.status} onChange={e => setFormData({...formData, status: e.target.value})} className="input-dark w-full mt-1 text-xs"><option value="active">Active</option><option value="inactive">Inactive</option><option value="incarcerated">Incarcerated</option><option value="deceased">Deceased</option></select></div>
              <div><label htmlFor="ff-gangintelpage-4" className="text-[9px] text-rmpg-400 uppercase font-bold">Threat Level</label><select id="ff-gangintelpage-4" value={formData.threat_level} onChange={e => setFormData({...formData, threat_level: e.target.value})} className="input-dark w-full mt-1 text-xs"><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option><option value="critical">Critical</option></select></div>
            </div>
            <div className="mt-3"><label htmlFor="ff-gangintelpage-5" className="text-[9px] text-rmpg-400 uppercase font-bold">Notes</label><textarea id="ff-gangintelpage-5" value={formData.notes} onChange={e => setFormData({...formData, notes: e.target.value})} className="input-dark w-full mt-1 text-xs" rows={3} /></div>
            <div className="flex justify-end gap-3 mt-4">
              <button onClick={() => setFormOpen(false)} className="toolbar-btn px-4" style={{ height: 28 }}>Cancel</button>
              <button onClick={handleSave} disabled={formSubmitting || !formData.name} className="toolbar-btn toolbar-btn-primary px-4" style={{ height: 28 }}>{formSubmitting ? 'Saving...' : 'Save'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {deleteId !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={() => setDeleteId(null)}>
          <div className="bg-surface-raised border border-red-800 p-6 max-w-sm w-full" style={{ borderRadius: 2 }} onClick={e => e.stopPropagation()}>
            ...
            <p className="text-xs text-rmpg-400 mb-4">This permanently removes this member record. This cannot be undone.</p>
