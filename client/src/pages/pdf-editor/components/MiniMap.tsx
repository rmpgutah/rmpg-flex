  ...
  return (
    <div className="fixed bottom-4 right-4 z-30 bg-surface-base border border-border-default rounded-[2px] shadow-lg">
      <div className="flex items-center gap-1.5 px-2 py-1 border-b border-border-default cursor-default">
        <MapIcon className="w-3 h-3 [color:var(--panel-header-color)]" />
        <span className="text-[10px] uppercase tracking-wider text-fg-muted font-semibold">Page navigator</span>
        <span className="text-[10px] text-fg-muted">{activePage} / {pageOrder.length}</span>
        <button type="button" onClick={() => setCollapsed(c => !c)}
          className="ml-auto text-[10px] text-fg-muted hover:text-rmpg-100 px-1"
          title={collapsed ? 'Expand' : 'Collapse'}>{collapsed ? '▴' : '▾'}</button>
        <button type="button" onClick={onClose} className="text-fg-muted hover:text-rmpg-100 p-0.5"
          ...
          {pageOrder.map((original, idx) => {
            ...
            return (
              <button
                ...
                className={`block w-[${THUMB_WIDTH}px] mx-auto p-0.5 rounded-sm border ${active ? '[border-color:var(--field-label-color)]' : 'border-border-default hover:border-rmpg-600'}`}
                style={{ width: THUMB_WIDTH }}
              >
                <div className="bg-white aspect-[3/4] flex items-center justify-center overflow-hidden">
                  {original === 0 ? (
                    <span className="text-[8px] text-fg-muted">Blank</span>
                  ) : (
                    <canvas
                ...
                <div className={`text-[9px] text-center mt-0.5 ${active ? '[color:var(--panel-header-color)] font-semibold' : 'text-fg-muted'}`}>
                  {visualPageNumber}
                </div>
              </button>
            );
