...
export default function PreferencesDialog({ open, prefs, onChange, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 flex items-start justify-center p-4 overflow-y-auto" onClick={onClose}>
      <div className="bg-surface-base border border-border-default rounded-[2px] p-4 max-w-[480px] w-full my-auto" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-semibold text-rmpg-100 inline-flex items-center gap-2">
            <Settings className="w-4 h-4 [color:var(--panel-header-color)]" /> Editor preferences
          </h3>
          <button type="button" onClick={onClose} className="p-1 text-rmpg-400 hover:text-rmpg-100" aria-label="Close preferences"><X className="w-4 h-4" /></button>
        </div>

        <div className="space-y-3">
          <div>
            <label htmlFor="ff-preferencesdialog-0" className={labelCls}>View mode</label>
          ...
          {prefs.snapToGrid && (
            <div>
              <label htmlFor="ff-preferencesdialog-2" className={labelCls}>Grid size (PDF points)</label>
        ...
        <div className="flex items-center justify-between gap-2 mt-4">
          <button type="button" onClick={() => onChange(DEFAULT_PREFERENCES)} className="text-[10px] text-rmpg-400 hover:text-rmpg-100">
            Restore defaults
          </button>
