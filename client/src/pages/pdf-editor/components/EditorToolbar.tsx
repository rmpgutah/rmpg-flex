import { Upload, Save, FilePlus, Combine, FileText, Settings2, Undo2, Redo2, ZoomIn, ZoomOut, Maximize2, Stamp, FolderUp, Lock, LockOpen, Sparkles, RotateCw, FlipVertical2, Hash, MoveHorizontal, FileDown } from 'lucide-react';
...
export default function EditorToolbar(p: Props) {
  const btn = 'p-1.5 rounded-sm text-fg-muted hover:text-rmpg-100 hover:bg-rmpg-700/60 transition-colors disabled:opacity-30 disabled:hover:bg-transparent';
  return (
    <div className="flex items-center gap-1 bg-surface-base border border-border-default rounded-[2px] px-2 py-1.5 flex-shrink-0 flex-wrap md:flex-nowrap">
      ...
      <span className="text-xs text-fg-muted truncate max-w-[260px] px-2" title={p.fileName}>{p.fileName || 'No document open'}</span>
      ...
      <button type="button" onClick={p.onZoomReset} disabled={!p.hasDocument}
        className="px-2 py-1 text-[10px] text-fg-muted hover:text-rmpg-100 hover:bg-rmpg-700/60 rounded-sm disabled:opacity-30 min-w-[48px]"
        title="Reset zoom"
      ...
      {p.onFitWidth && (
        <IconButton onClick={p.onFitWidth} aria-label="Fit width" title="Fit to width (2)" className={btn} disabled={!p.hasDocument}><MoveHorizontal className="w-4 h-4" /></IconButton>
      )}
      {p.onFitPage && (
      ...
      {p.onReversePages && (
        <IconButton onClick={p.onReversePages} aria-label="Reverse page order" title="Reverse page order" className={btn} disabled={!p.hasDocument}><FlipVertical2 className="w-4 h-4" /></IconButton>
      )}
      {p.onTogglePageNumbers && (
        <IconButton onClick={p.onTogglePageNumbers}
          ...
          className={`p-1.5 rounded-sm transition-colors ${p.pageNumbersActive ? 'bg-[#d4a017]/20 [color:var(--panel-header-color)]' : 'text-fg-muted hover:text-rmpg-100 hover:bg-rmpg-700/60'}`}
      ...
      {p.onEncrypt && (
        <IconButton
          ...
          className={`p-1.5 rounded-sm transition-colors ${p.encryptionActive ? 'bg-[#d4a017]/20 [color:var(--panel-header-color)]' : 'text-fg-muted hover:text-rmpg-100 hover:bg-rmpg-700/60'}`}
      ...
      {p.onDownloadFlattened && (
        <IconButton onClick={p.onDownloadFlattened} aria-label="Download flattened copy" title="Download a flattened copy (annotations baked into the page content)" className={btn} disabled={!p.hasDocument}><FileDown className="w-4 h-4" /></IconButton>
      )}
      {p.onSaveToDocuments && (
