...
interface Props {
  ...
  activePage: number;
  pageCount: number;
  onAdd: (title: string, page: number) => void;
...
export default function BookmarksPanel({ bookmarks, activePage, pageCount, onAdd, onDelete, onJump, onClose }: Props) {
  ...
  return (
    <div className="bg-[#0d0d0d] border border-[#222222] rounded-[2px] w-[220px] flex-shrink-0 p-2 overflow-y-auto">
      ...
      {sorted.length === 0 ? (
        <div className="text-[10px] text-rmpg-600 px-1 py-2">No bookmarks. Add one to jump quickly between pages.</div>
      ) : (
        <div className="space-y-0.5">
          {sorted.map(b => (
            <div key={b.id} className={`group flex items-center gap-1 rounded-sm px-1.5 py-1 ${b.page === activePage ? 'bg-[#d4a017]/15' : 'hover:bg-rmpg-700/30'}`}>
