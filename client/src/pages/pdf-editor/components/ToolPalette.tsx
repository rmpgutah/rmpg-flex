import { MousePointer2, Hand, Type, Highlighter, Underline, Square, Circle, Minus, MoveUpRight, Pencil, PenTool, Image as ImageIcon, Stamp, EyeOff, Link2, Crop, QrCode, StickyNote, Calendar, Pentagon, Spline, Cloud, Check, X, Ruler, TextCursorInput, SquareCheck, Shapes, ChevronDownSquare, CircleDot, CalendarClock } from 'lucide-react';
...
const TOOLS: { id: Tool; icon: React.ComponentType<{ className?: string }>; label: string }[] = [
  ...
  { id: 'strikethrough', icon: () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M16 4H9a3 3 0 0 0-2.83 4"/><path d="M14 12a4 4 0 0 1 0 8H6"/><line x1="4" x2="20" y1="12" y2="12"/></svg>, label: 'Strikethrough' },
      ...
      {TOOLS.map(t => {
        ...
        return (
          <IconButton
            ...
            aria-label={t.label}
            title={t.label}
            className={`p-1.5 rounded-sm transition-colors ${active ? 'bg-[#d4a017]/20 [color:var(--panel-header-color)]' : 'text-rmpg-400 hover:text-rmpg-100 hover:bg-rmpg-700/50'}`}
          >
