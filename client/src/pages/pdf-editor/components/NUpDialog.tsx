  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div className="bg-surface-base border border-border-default rounded-[2px] w-[360px] p-4 shadow-xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="text-[10px] text-fg-muted mb-3">Combine multiple source pages onto each printed sheet. Annotations are baked in first.</div>

        <label className="text-[9px] uppercase tracking-wider text-fg-muted block mb-1">Pages per sheet</label>
        <div className="flex gap-1 mb-3">
          {([2, 4] as const).map(n => (
            <button key={n} type="button" onClick={() => setUp(n)}
              className={`flex-1 px-2 py-1.5 text-[11px] rounded-sm border ${up === n ? 'bg-[#d4a017]/20 [color:var(--panel-header-color)] [border-color:var(--field-label-color)]' : 'border-border-default text-fg-muted hover:text-rmpg-100'}`}>
              {n}-up
            </button>
          ))}
        </div>

        <label className="text-[9px] uppercase tracking-wider text-fg-muted block mb-1">Sheet size</label>
        <div className="flex gap-1 mb-3">
          {(['Letter', 'A4'] as const).map(s => (
            <button key={s} type="button" onClick={() => setSize(s)}
              className={`flex-1 px-2 py-1.5 text-[11px] rounded-sm border ${size === s ? 'bg-[#d4a017]/20 [color:var(--panel-header-color)] [border-color:var(--field-label-color)]' : 'border-border-default text-fg-muted hover:text-rmpg-100'}`}>
              {s}
            </button>
          ))}
        </div>

        <div className="text-[10px] text-fg-muted mb-3">{pageCount} page{pageCount === 1 ? '' : 's'} → <span className="[color:var(--panel-header-color)] font-mono">{sheets}</span> sheet{sheets === 1 ? '' : 's'} ({size}{up === 2 ? ' landscape' : ''}).</div>
