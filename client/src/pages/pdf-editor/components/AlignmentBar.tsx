...
export default function AlignmentBar({ count, onAlign, onDistribute, onMatchSize }: Props) {
  const btn = 'p-1 hover:bg-rmpg-700/40 rounded-sm text-fg-muted hover:[color:var(--panel-header-color)] disabled:opacity-30 disabled:hover:bg-transparent disabled:hover:text-fg-muted';
  ...
  return (
    <div className="inline-flex items-center gap-0.5 border-l border-border-default pl-2 ml-1" role="group" aria-label="Arrange selection">
      ...
      <span className="w-px h-3.5 bg-surface-raised mx-0.5" />
