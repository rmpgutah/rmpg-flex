...
export default function MdtPage() {
  ...
  const [activeTab, setActiveTab] = useState<'my-calls' | 'pending' | 'messages'>('my-calls');
  ...
  return (
    <div className="h-full flex flex-col bg-surface-base text-white overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* ── LEFT: Call List ── */}
        <div className="w-2/5 flex flex-col border-r border-rmpg-700/50 overflow-hidden">
          {/* Tabs */}
          <div className="flex border-b border-rmpg-700/50 flex-shrink-0" style={{ background: '#111' }}>
            {(['my-calls', 'pending', 'messages'] as const).map(tab => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className="flex-1 px-3 py-1.5 text-[9px] font-bold uppercase tracking-wider transition-colors"
                style={{
          ...
          {selectedCall ? (
            <>
              ...
              <div className="flex-1 overflow-auto p-4 space-y-3">
                ...
                {/* Subject / Vehicle descriptions */}
                {selectedCall.subject_description && (
                  <div>
                    <div className="text-[9px] text-rmpg-500 uppercase font-bold tracking-wider mb-1">Subject Description</div>
                    <div className="text-[10px] text-rmpg-200">{selectedCall.subject_description}</div>
                  </div>
                )}
                {selectedCall.vehicle_description && (
                  <div>
                    <div className="text-[9px] text-rmpg-500 uppercase font-bold tracking-wider mb-1">Vehicle Description</div>
