...
describe('Map dock render (registry-driven)', () => {
  ...
  it('renders exactly 55 switch rows across both docks whose accessible names match the registry labels', () => {
    const bindings = buildAllBoundBindings();
    ...
    expect(switches).toHaveLength(55);
  ...
  it('gives every one of the 55 rows a leading lucide <svg> icon (never the loading/error icon)', () => {
    const bindings = buildAllBoundBindings();
    const leftSections = buildDockSections(LEFT_DOCK_GROUPS, bindings);
    const rightSections = buildDockSections(RIGHT_DOCK_GROUPS, bindings);
    render(
