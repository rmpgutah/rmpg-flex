...
// Collapsible panel with a horizontal bar chart showing beat
...
export default function CoverageTimeline({ data, expanded, onToggle }: CoverageTimelineProps) {
  ...
  const beatFreq = new Map<string, { name: string; count: number }>();
  ...
  const intervalMaps = intervals.map((iv) => {
    const m = new Map<string, { unit_count: number; avg_speed: number | null }>();
    for (const z of iv.zones) {
          ...
          <div className="overflow-x-auto" style={{ minWidth: 400 }}>
            ...
            <div className="flex" style={{ marginLeft: 80 }}>
              {intervals.map((iv, i) => (
                <div
                  key={i}
                  className="text-center font-mono text-rmpg-500"
                  style={{ width: 18, minWidth: 18, fontSize: 7, lineHeight: '12px' }}
                >
            ...
            {topBeats.map((beat) => (
              <div key={beat.id} className="flex items-center" style={{ height: 14 }}>
                {/* Beat name */}
                <div
                  className="font-mono truncate text-rmpg-500"
                  style={{ width: 80, minWidth: 80, fontSize: 7, paddingRight: 4 }}
                  title={beat.name}
                >
                  {beat.name}
                </div>
                ...
                {intervals.map((iv, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      title={tooltip}
                      style={{ width: 18, minWidth: 18, height: 10, backgroundColor: bgColor, opacity, borderRadius: 1, margin: '0 0.5px' }}
                    />
                  );
                })}
              </div>
