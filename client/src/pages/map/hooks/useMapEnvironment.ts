// ============================================================
// RMPG Flex — useMapEnvironment Hook
// Environmental safety conditions: lighting, weather hazards,
// wind, visibility, icy roads, and school zone awareness.
// Data-only hook — no map rendering.
// ============================================================
...
function findNextTransition(
  ...
  lat: number,
  lng: number,
): { minutes: number; type: 'sunrise' | 'sunset' } {
  ...
  for (let ms = startMs + STEP_MS; ms <= startMs + MAX_MS; ms += STEP_MS) {
    ...
    if (prevElev <= 0 && elev > 0) {
      return { minutes: Math.round((ms - startMs) / 60000), type: 'sunrise' };
    }
    if (prevElev >= 0 && elev < 0) {
      return { minutes: Math.round((ms - startMs) / 60000), type: 'sunset' };
    }
    prevElev = elev;
  }
...
function isSchoolZoneTime(): boolean {
  ...
  // 7:30-8:30 AM and 2:30-3:30 PM
  return (totalMin >= 450 && totalMin <= 510) || (totalMin >= 870 && totalMin <= 930);
}

// ─── Default state ──────────────────────────────────────────

const DEFAULT_WEATHER: WeatherHazards = {
  freezing: false,
  highWind: false,
  rain: false,
  snow: false,
...
export function useMapEnvironment(
  ...
  const calculate = useCallback(async () => {
    ...
    // Solar calculations (client-side)
    const now = new Date();
    ...
    setSunriseSunset({
      sunrise: '',
      sunset: '',
      minutesToNextTransition: transition.minutes,
      nextTransition: transition.type,
    });

    // School zone check
    setSchoolZoneActive(isSchoolZoneTime());

    // Try server-side lighting API (may have additional data)
    try {
      const lightData = await apiFetch<{
        condition: LightingCondition;
        sunrise: string;
        sunset: string;
      }>('/map/safety/lighting-conditions');
      if (lightData?.condition) {
        ...
        if (lightData.sunrise || lightData.sunset) {
          setSunriseSunset((prev) =>
            prev
              ? { ...prev, sunrise: lightData.sunrise || '', sunset: lightData.sunset || '' }
              : prev,
          );
        }
      }
    ...
    try {
      ...
      if (res.ok) {
        ...
        setWeatherHazards({
          ...
          rain,
          snow,
          description: parts.length > 0 ? parts.join(', ') : 'Clear',
        });
        ...
        setWindCondition({
          speed: windSpeed,
          direction: windDir,
          cardinal: windCardinal(windDir),
        });

        // Estimate visibility
        let vis = 10000; // 10km base
        ...
        // Low visibility includes weather-degraded conditions
