...
export default function BoloCard({ bolo, canDelete, onResolve, onDelete }: {
  ...
  return (
    <div className={`border rounded-[2px] p-3 ${expired ? 'border-border-default bg-[#040404] opacity-60' : 'border-border-default bg-surface-overlay'}`}>
      ...
      {!expired && (
        <div className="flex gap-2 mt-2 justify-end">
          <button onClick={() => onResolve(bolo.id)}
            className="font-mono text-[8px] tracking-wide text-[#d4a017] border border-border-subtle rounded-[2px] px-2 py-[5px] uppercase">Resolve</button>
          {canDelete && (
