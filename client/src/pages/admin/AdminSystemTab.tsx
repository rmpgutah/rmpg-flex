...
const DEFAULT_CALL_SOURCES = ['phone', 'radio', 'walk_in', 'alarm', 'patrol', 'online', 'dispatch', 'email', 'other'];
...
export default function AdminSystemTab({
  ...
  return (
    <div className="flex h-full overflow-hidden">
      ...
      <div className="flex-shrink-0 border-r border-rmpg-700 overflow-y-auto bg-surface-sunken" style={{ width: 220 }}>
        <div className="px-3 py-2.5 border-b border-rmpg-700">
          <div className="text-[9px] font-bold text-rmpg-400 uppercase tracking-widest">Configuration</div>
          {hasUnsaved && (
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-2 px-3 py-2 text-left text-[10px] transition-colors ${
                  isActive
                    ? 'bg-brand-900/30 text-white border-l-2 border-l-brand-500'
                    : 'text-rmpg-400 hover:bg-rmpg-700/30 hover:text-rmpg-200 border-l-2 border-l-transparent'
                }`}
              >
                <Icon className={`w-3.5 h-3.5 flex-shrink-0 ${isActive ? 'text-brand-400' : ''}`} />
                <span className="flex-1 truncate font-medium">{sec.label}</span>
                {sectionBadges[sec.id] !== undefined && (
                  <span className={`text-[8px] font-mono px-1.5 py-0.5 ${isActive ? 'bg-brand-900/50 text-brand-400' : 'bg-rmpg-700 text-rmpg-500'}`}>
                    {sectionBadges[sec.id]}
                  </span>
                )}
              </button>
            );
