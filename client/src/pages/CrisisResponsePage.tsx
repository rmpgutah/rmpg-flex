        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={() => setFormOpen(false)}>
          <div className="bg-surface-raised border border-rmpg-700 p-6 max-w-lg w-full" style={{ borderRadius: 2 }} onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="col-span-2"><label className="text-[9px] text-rmpg-400 uppercase font-bold">Disposition</label><input value={formData.disposition} onChange={e => setFormData({...formData, disposition: e.target.value})} className="input-dark w-full mt-1 text-xs" /></div>
              <div className="flex items-center gap-4 col-span-2">
                <label className="flex items-center gap-2 text-[11px] text-rmpg-300"><input type="checkbox" checked={formData.cit_team_used === 1} onChange={e => setFormData({...formData, cit_team_used: e.target.checked ? 1 : 0})} className="w-3 h-3" /> CIT Team Used</label>
                <label className="flex items-center gap-2 text-[11px] text-rmpg-300"><input type="checkbox" checked={formData.resolved_on_scene === 1} onChange={e => setFormData({...formData, resolved_on_scene: e.target.checked ? 1 : 0})} className="w-3 h-3" /> Resolved on Scene</label>
                <label className="flex items-center gap-2 text-[11px] text-rmpg-300"><input type="checkbox" checked={formData.diverted === 1} onChange={e => setFormData({...formData, diverted: e.target.checked ? 1 : 0})} className="w-3 h-3" /> Diverted</label>
              </div>
            </div>
