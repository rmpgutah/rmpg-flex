...
export default function StatuteAnalyticsPage() {
  ...
  return (
    <div className="h-full flex flex-col bg-surface-base text-rmpg-100 overflow-hidden">
      ...
      {/* Mobile: day selector */}
      {isMobile && (
        <div className="flex items-center gap-1 px-3 py-2 overflow-x-auto flex-shrink-0" style={{ background:"var(--surface-sunken)", borderBottom: '1px solid var(--border-default)' }}>
          {[30, 60, 90, 180, 365].map(d => (
            <button type="button"
              key={d}
              onClick={() => setDays(d)}
            ...
            <div className="panel-surface p-3">
              ...
              <div className="space-y-2">
                {byLevel.map((l, i) => {
                  ...
                  const color = levelColors[l.offense_level] || 'var(--rmpg-500)';
                  return (
                    <div key={i}>
