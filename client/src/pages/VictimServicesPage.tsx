...
export default function VictimServicesPage() {
  ...
  // Track the full row, not just the id, so the destructive modal can
  // surface name + case_number + crime_type — audit caught the bare
  // "Delete Victim Record?" prompt as a wrong-row hazard (2026-06-21).
  ...
  const columns = [
    ...
    { key: 'status', label: 'Status', render: (r: Victim) => <span className={`badge ${r.status === 'active' ? 'badge-p1' : 'badge-p4'}`}>{formatEnumValue(r.status)}</span> },
  ...
  if (loading) return <div className="p-6 text-rmpg-500">Loading victim services...</div>;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={() => setFormOpen(false)}>
          <div className="bg-surface-raised border border-rmpg-700 p-6 max-w-lg w-full" style={{ borderRadius: 2 }} onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label htmlFor="ff-victimservicespage-0" className="text-[9px] text-rmpg-400 uppercase font-bold">Name *</label><input id="ff-victimservicespage-0" value={formData.victim_name} onChange={e => setFormData({...formData, victim_name: e.target.value})} className="input-dark w-full mt-1 text-xs" /></div>
              <div><label htmlFor="ff-victimservicespage-1" className="text-[9px] text-rmpg-400 uppercase font-bold">Case #</label><input id="ff-victimservicespage-1" value={formData.case_number} onChange={e => setFormData({...formData, case_number: e.target.value})} className="input-dark w-full mt-1 text-xs" /></div>
              <div><label htmlFor="ff-victimservicespage-2" className="text-[9px] text-rmpg-400 uppercase font-bold">Crime Type</label><input id="ff-victimservicespage-2" value={formData.crime_type} onChange={e => setFormData({...formData, crime_type: e.target.value})} className="input-dark w-full mt-1 text-xs" /></div>
              <div><label htmlFor="ff-victimservicespage-3" className="text-[9px] text-rmpg-400 uppercase font-bold">Status</label><select id="ff-victimservicespage-3" value={formData.status} onChange={e => setFormData({...formData, status: e.target.value})} className="input-dark w-full mt-1 text-xs"><option value="active">Active</option><option value="closed">Closed</option><option value="referred">Referred</option><option value="pending">Pending</option></select></div>
              <div><label htmlFor="ff-victimservicespage-4" className="text-[9px] text-rmpg-400 uppercase font-bold">Phone</label><input id="ff-victimservicespage-4" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} className="input-dark w-full mt-1 text-xs" /></div>
              <div><label htmlFor="ff-victimservicespage-5" className="text-[9px] text-rmpg-400 uppercase font-bold">Email</label><input id="ff-victimservicespage-5" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className="input-dark w-full mt-1 text-xs" /></div>
              <div className="col-span-2"><label htmlFor="ff-victimservicespage-6" className="text-[9px] text-rmpg-400 uppercase font-bold">Address</label><input id="ff-victimservicespage-6" value={formData.address} onChange={e => setFormData({...formData, address: e.target.value})} className="input-dark w-full mt-1 text-xs" /></div>
              <div className="flex items-center gap-4 col-span-2">
                <label htmlFor="ff-victimservicespage-7" className="flex items-center gap-2 text-[11px] text-rmpg-300"><input id="ff-victimservicespage-7" type="checkbox" checked={formData.safety_plan === 1} onChange={e => setFormData({...formData, safety_plan: e.target.checked ? 1 : 0})} className="w-3 h-3" /> Safety Plan</label>
                <label htmlFor="ff-victimservicespage-8" className="flex items-center gap-2 text-[11px] text-rmpg-300"><input id="ff-victimservicespage-8" type="checkbox" checked={formData.protective_order === 1} onChange={e => setFormData({...formData, protective_order: e.target.checked ? 1 : 0})} className="w-3 h-3" /> Protective Order</label>
              </div>
            </div>
            <div className="mt-3"><label htmlFor="ff-victimservicespage-9" className="text-[9px] text-rmpg-400 uppercase font-bold">Notes</label><textarea id="ff-victimservicespage-9" value={formData.notes} onChange={e => setFormData({...formData, notes: e.target.value})} className="input-dark w-full mt-1 text-xs" rows={3} /></div>
            <div className="flex justify-end gap-3 mt-4">
              <button onClick={() => setFormOpen(false)} className="toolbar-btn px-4" style={{ height: 28 }}>Cancel</button>
              <button onClick={handleSave} disabled={formSubmitting || !formData.victim_name} className="toolbar-btn toolbar-btn-primary px-4" style={{ height: 28 }}>{formSubmitting ? 'Saving...' : 'Save'}</button>
            </div>
          </div>
        </div>
      )}
