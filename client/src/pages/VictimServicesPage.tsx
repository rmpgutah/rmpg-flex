        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={() => setFormOpen(false)}>
          <div className="bg-surface-raised border border-rmpg-700 p-6 max-w-lg w-full" style={{ borderRadius: 2 }} onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className="text-[9px] text-rmpg-400 uppercase font-bold">Name *</label><input value={formData.victim_name} onChange={e => setFormData({...formData, victim_name: e.target.value})} className="input-dark w-full mt-1 text-xs" /></div>
              ...
              <div><label className="text-[9px] text-rmpg-400 uppercase font-bold">Crime Type</label><input value={formData.crime_type} onChange={e => setFormData({...formData, crime_type: e.target.value})} className="input-dark w-full mt-1 text-xs" /></div>
              <div><label className="text-[9px] text-rmpg-400 uppercase font-bold">Status</label><select value={formData.status} onChange={e => setFormData({...formData, status: e.target.value})} className="input-dark w-full mt-1 text-xs"><option value="active">Active</option><option value="closed">Closed</option><option value="referred">Referred</option><option value="pending">Pending</option></select></div>
              ...
              <div className="flex items-center gap-4 col-span-2">
                <label className="flex items-center gap-2 text-[11px] text-rmpg-300"><input type="checkbox" checked={formData.safety_plan === 1} onChange={e => setFormData({...formData, safety_plan: e.target.checked ? 1 : 0})} className="w-3 h-3" /> Safety Plan</label>
                <label className="flex items-center gap-2 text-[11px] text-rmpg-300"><input type="checkbox" checked={formData.protective_order === 1} onChange={e => setFormData({...formData, protective_order: e.target.checked ? 1 : 0})} className="w-3 h-3" /> Protective Order</label>
              </div>
            </div>
