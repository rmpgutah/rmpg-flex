...
export type CommandAction =
  ...
  | { type: 'query_nhtsa_vin'; query: string }
  | { type: 'query_fmcsa_carrier'; query: string }
  | { type: 'query_criminal'; query: string }
  | { type: 'query_business'; query: string }
  | { type: 'query_individual'; query: string }
  | { type: 'query_phone'; query: string }
...
const COMMANDS: Record<string, { usage: string; desc: string }> = {
  ...
  QN:   { usage: 'QN <VIN>',                  desc: 'NHTSA VIN report (federal)' },
  QC:   { usage: 'QC <DOT#>',                 desc: 'FMCSA carrier lookup (federal)' },
  QX:   { usage: 'QX <name>',                 desc: 'Criminal records search (multi-DB)' },
  QE:   { usage: 'QE <company>',             desc: 'Business entity lookup (OpenCorporates)' },
  QI:   { usage: 'QI <name>',               desc: 'Individual person lookup (Enformion)' },
  QZ:   { usage: 'QZ <phone>',              desc: 'Reverse phone lookup (Enformion)' },
  ...
  LE:   { usage: 'LE <call#> [agency]',      desc: 'Notify law enforcement agency' },
...
export async function executeCommand(
  ...
  switch (verb) {
    ...
    // ── NHTSA VIN Report (Federal) ──
    case 'QN': {
      if (args.length < 1) {
        return { success: false, message: 'Usage: QN <VIN>', action: { type: 'none' } };
      }
      const query = args[0].toUpperCase();
      return {
        success: true,
        message: `Querying NHTSA VIN: ${query}...`,
        action: { type: 'query_nhtsa_vin', query },
      };
    }

    // ── FMCSA Carrier Lookup (Federal) ──
    case 'QC': {
      if (args.length < 1) {
        return { success: false, message: 'Usage: QC <DOT#>', action: { type: 'none' } };
      }
      const query = args[0];
      return {
        success: true,
        message: `Querying FMCSA carrier: ${query}...`,
        action: { type: 'query_fmcsa_carrier', query },
      };
    }
    ...
    case 'QX': {
      if (args.length < 1) {
        return { success: false, message: 'Usage: QX <name>', action: { type: 'none' } };
      }
      const query = args.join(' ');
      return {
        success: true,
        message: `Querying criminal records: ${query}...`,
        action: { type: 'query_criminal', query },
      };
    }
    ...
    case 'QE': {
      if (args.length < 1) {
        return { success: false, message: 'Usage: QE <company name>', action: { type: 'none' } };
      }
      const query = args.join(' ');
      return {
        success: true,
        message: `Querying business records: ${query}...`,
        action: { type: 'query_business', query },
      };
    }
    ...
    case 'QI': {
      if (args.length < 1) {
        return { success: false, message: 'Usage: QI <name>', action: { type: 'none' } };
      }
      const query = args.join(' ');
      return {
        success: true,
        message: `Querying individual (Enformion): ${query}...`,
        action: { type: 'query_individual', query },
      };
    }
    ...
    case 'QZ': {
      if (args.length < 1) {
        return { success: false, message: 'Usage: QZ <phone>', action: { type: 'none' } };
      }
      const query = args.join(' ');
      return {
        success: true,
        message: `Querying reverse phone (Enformion): ${query}...`,
        action: { type: 'query_phone', query },
      };
    }
