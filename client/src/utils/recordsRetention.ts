...
export function trackArchiveRetrievals(archives:ArchiveRecord[]): { total:number; eligible:number; destroyed:number } { const now=new Date(); return { total:archives.length, eligible:archives.filter(a=>parseTimestamp(a.retentionExpires)<now).length, destroyed:archives.filter(a=>a.status==='destroyed').length }; }
...
export function planInventoryCycle(inventory:RecordsInventory[]): { totalRecords:number; needsCounting:number } { return { totalRecords:inventory.reduce((s,i)=>s+i.physicalCount+i.digitalCount,0), needsCounting:inventory.filter(i=>parseTimestamp(i.nextCountDate)<new Date()).length }; }
