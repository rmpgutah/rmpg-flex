...
export function checkIndexHealth(indexes:SearchIndex[]): {totalDocuments:number;staleIndexes:SearchIndex[]} { const now=Date.now(); return{totalDocuments:indexes.reduce((s,i)=>s+i.documentCount,0),staleIndexes:indexes.filter(i=>now-parseTimestamp(i.lastIndexed).getTime()>86400000)}; }
...
export function checkCacheValidity(cache:SearchCache): boolean { return (Date.now()-parseTimestamp(cache.cachedAt).getTime())/1000<cache.ttlSeconds; }
