...
//
// The notice is RESPONDENT-facing (left at the door / handed to the person
// being served) — these tests also pin that the serve-job enrichment fields
// (respondent name, court case, document type) flow through the mapper.
...
describe('buildNoticeOfCommunicationFromCall', () => {
  it('populates the payload from the failed CFS row', () => {
    ...
    // the generator falls back to "Occupant / Respondent" wording.
    expect(data.respondentName).toBeUndefined();
    expect(data.courtCaseNumber).toBeUndefined();
  });

  it('surfaces the respondent + legal matter from the linked serve job', () => {
    const data = buildNoticeOfCommunicationFromCall(
      failedCall,
      { officerName: 'C. Zamora', officerBadge: '5721' },
      {
        respondentName: 'Alejandro Vargas',
        documentType: 'summons',
        courtCaseNumber: '260904298',
        courtName: 'Third Judicial District Court, State of Utah',
      },
    );
    expect(data.respondentName).toBe('Alejandro Vargas');
    expect(data.courtCaseNumber).toBe('260904298');
    expect(data.courtName).toContain('Third Judicial District');
    // document_type 'summons' → respondent-readable "Summons Service"
    expect(data.serviceType).toBe('Summons Service');
  });
});
...
describe('generateNoticeOfCommunication', () => {
  it('emits a real PDF document with content (not a blank shell)', async () => {
    const data = buildNoticeOfCommunicationFromCall(
      failedCall,
      { officerName: 'C. Zamora', officerBadge: '5721' },
      { respondentName: 'Alejandro Vargas', documentType: 'summons', courtCaseNumber: '260904298' },
    );
  ...
  it('renders without a serve job (occupant fallback)', async () => {
    const data = buildNoticeOfCommunicationFromCall(failedCall, {
      officerName: 'C. Zamora',
      officerBadge: '5721',
    });
...
describe('psoResultLabel', () => {
  it('maps the live disposition codes to respondent-readable text', () => {
    ...
    expect(psoResultLabel('PS Sub-Served')).toBe('Delivered to co-resident / agent');
