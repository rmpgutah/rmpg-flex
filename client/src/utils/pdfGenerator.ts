...
export function addReportHeader(
  ...
  doc.text(
    ...
    textStartX, LAYOUT.HEADER_TOP + 22,
  );

  // ── Case number box (right) — single clean border ──────
  ...
  doc.text(caseNumber, caseBoxX + LAYOUT.CASE_BOX_W / 2, caseBoxY + 15, { align: 'center' });
...
export function openAutoSection(doc: jsPDF, title: string, y: number): { contentY: number; sectionY: number } {
  ...
  // Black header bar with white text — always black regardless of branding
  ...
  doc.text(title.toUpperCase(), LAYOUT.PAGE_MARGIN + SPACING.CONTENT_INSET, y + 5.8);
 ...
 */
export function addSignatureBlock(doc: jsPDF, roleLabel: string, x: number, y: number, width: number): number {
  // @ts-expect-error jsPDF GState — ensure full opacity
  doc.setGState(new doc.GState({ opacity: 1.0 }));
  ...
  // Role label in black header bar
...
/**
 * Two-line footer with form number and classification.
 */
export function addPageFooter(doc: jsPDF, pageNum: number, totalPages: number, formKey?: string) {
  ...
  // Ensure full opacity (safety reset — footer runs after watermark in final loop)
  // @ts-expect-error jsPDF GState
  doc.setGState(new doc.GState({ opacity: 1.0 }));

  // Dark footer bar — single line, no accent
  const footerY = pageHeight - LAYOUT.FOOTER_HEIGHT - SPACING.MD;
  doc.setFillColor(headerBg[0], headerBg[1], headerBg[2]);
  doc.rect(LAYOUT.PAGE_MARGIN, footerY, cw, LAYOUT.FOOTER_HEIGHT, 'F');

  // Single line: Form # | Rev | LAW ENFORCEMENT SENSITIVE | Page X of Y
  doc.setFont('helvetica', 'bold');
  ...
  if (formNum) {
    doc.text(`${formNum}  |  Rev. ${FORM_REVISION}`, LAYOUT.PAGE_MARGIN + SPACING.MD, lineY);
  }
  doc.text('LAW ENFORCEMENT SENSITIVE', pageWidth / 2, lineY, { align: 'center' });
  doc.text(`Page ${pageNum} of ${totalPages}`, pageWidth - LAYOUT.PAGE_MARGIN - SPACING.MD, lineY, { align: 'right' });
}
...
export function checkPageBreak(doc: jsPDF, y: number, needed: number, priority?: string): number {
  ...
  if (y + needed > pageHeight - LAYOUT.FOOTER_HEIGHT - 12) {
    ...
    const contY = 8;
    // Black continuation bar — matches section headers
    doc.setFillColor(0, 0, 0);
...
export function addTableWithShading(
  ...
  const drawHeaders = (atY: number): number => {
    const headerRowH = 8;
    // Black table header bar — matches section headers
    doc.setFillColor(0, 0, 0);
...
function generateGeneralIncident(doc: jsPDF, data: IncidentData) {
  ...
  { const sec = openAutoSection(doc, 'Flags', y); y = sec.contentY;
    ...
    flagX = addCheckboxField(doc, 'Domestic Violence', !!data.domestic_violence, flagX + SPACING.SM, y);
...
function generateTrespassWarning(doc: jsPDF, data: IncidentData) {
  ...
  { const sec = openAutoSection(doc, 'Signatures', y); y = sec.contentY;
    ...
    addSignatureBlock(doc, 'Issuing Officer', lx, y, hfw);
...
function generateAccidentReport(doc: jsPDF, data: IncidentData) {
  ...
  y = checkPageBreak(doc, y, 40, data.priority);
  { const sec = openAutoSection(doc, 'Signatures', y); y = sec.contentY;
    addSignatureBlock(doc, 'Investigating Officer', lx, y, hfw);
    y = addSignatureBlock(doc, 'Supervisor Review', rx, y, hfw);
    y = closeAutoSection(doc, sec.sectionY, y);
  }
}
...
function generateMedicalReport(doc: jsPDF, data: IncidentData) {
  ...
  { const sec = openAutoSection(doc, 'Signatures', y); y = sec.contentY;
    addSignatureBlock(doc, 'Responding Officer', lx, y, hfw);
    y = addSignatureBlock(doc, 'Supervisor Review', rx, y, hfw);
...
function generateDailyActivityReport(doc: jsPDF, data: IncidentData) {
  ...
  y = checkPageBreak(doc, y, 40, data.priority);
  { const sec = openAutoSection(doc, 'Signatures', y); y = sec.contentY;
    addSignatureBlock(doc, 'Officer', lx, y, hfw);
    y = addSignatureBlock(doc, 'Supervisor Review', rx, y, hfw);
    y = closeAutoSection(doc, sec.sectionY, y);
  }
}
...
function generateArrestReport(doc: jsPDF, data: IncidentData) {
  ...
  y = checkPageBreak(doc, y, 40, data.priority);
  { const sec = openAutoSection(doc, 'Signatures', y); y = sec.contentY;
    addSignatureBlock(doc, 'Arresting Officer', lx, y, hfw);
