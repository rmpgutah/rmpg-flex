...
export function buildTimeline(events:Array<{timestamp:string;eventType:string;source:string;description:string;artifact:string}>): DigitalTimeline { return{deviceId:'',events:events.sort((a,b)=>parseTimestamp(a.timestamp).getTime()-parseTimestamp(b.timestamp).getTime())}; }
