...
export function briefingsToCsv(rows: Array<{
  briefing_number: string; title: string; shift_type: string;
  ...
  return rowsToCsv(['number', 'title', 'shift', 'author', 'created_at', 'acked', 'roster'],
    rows.map((r) => [r.briefing_number, r.title, r.shift_type, r.created_by ?? r.created_by_name ?? '', r.created_at, r.acknowledged_count, r.total_officers]));
}
...
export function shiftNotesToCsv(rows: Array<{
  ...
  return rowsToCsv(['shift_date', 'officer', 'visibility', 'tags', 'created_at', 'content'],
    rows.map((r) => [r.shift_date ?? '', r.officer_name, r.visibility, (r.tags ?? []).join('|'), r.created_at, r.content]));
}
...
export function formatRadioLine(u: {
  ...
  if (u.badge) {
    return [u.unit_id, u.officer_name, `#${u.badge}`, u.status, u.current_call_number, u.location_description]
...
export function unitsBoardToTsv(rows: Array<{
  unit_id: string; officer_name: string; badge?: string; status: string;
  current_call_number?: string | null; location_description?: string | null;
}>): string {
  return [
    ['unit', 'officer', 'badge', 'status', 'call', 'location'].join('\t'),
    ...rows.map((r) => [r.unit_id, r.officer_name, r.badge ?? '', r.status, r.current_call_number ?? '', r.location_description ?? '']
...
export function trainingCoursesToCsv(rows: Array<{
  ...
  duration_hours?: string | number; location?: string;
  instructor?: string; instructor_name?: string;
  is_mandatory?: number | boolean;
}>): string {
  return rowsToCsv(['name', 'code', 'category', 'hours', 'location', 'instructor', 'mandatory'],
    rows.map((r) => [
      r.course_name ?? '', r.course_code ?? '', r.category ?? '', r.duration_hours ?? '',
      r.location ?? '', r.instructor_name ?? r.instructor ?? '', r.is_mandatory ? 'yes' : 'no',
    ]));
}
