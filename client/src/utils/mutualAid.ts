...
export function checkAgreementStatus(agreements:MutualAidAgreement[]): {active:number;expired:number;availableResources:Record<string,number>} { const now=new Date();const active=agreements.filter(a=>parseTimestamp(a.expirationDate)>now); const ar:Record<string,number>={};for(const a of active)for(const r of a.resourcesAvailable)ar[r]=(ar[r]||0)+1; return{active:active.length,expired:agreements.length-active.length,availableResources:ar}; }
...
export function compileAidDashboard(agreements:MutualAidAgreement[],requests:AidRequest[],deployments:AidDeployment[]): MutualAidDashboard { return{activeAgreements:agreements.filter(a=>parseTimestamp(a.expirationDate)>new Date()).length,pendingRequests:requests.filter(r=>r.status==='pending').length,deploymentsActive:deployments.filter(d=>!d.returnedAt).length,totalCostsYTD:deployments.reduce((s,d)=>s+d.cost,0)}; }
