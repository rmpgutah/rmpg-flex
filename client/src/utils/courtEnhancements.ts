...
export function logSubpoenaService(subpoenaId:string,method:string,server:string): SubpoenaService { return{subpoenaId,serviceMethod:method,serviceDate:localToday(),servedBy:server,proofOfService:true}; }
...
export function fileAppeal(caseNumber:string,grounds:string,appellant:string): CourtAppeal { return{caseNumber,appealDate:localToday(),appellant,grounds,court:'',status:'filed'}; }
...
export function postBond(caseNumber:string,defendant:string,amount:number,type:string): CourtBond { return{caseNumber,defendantName:defendant,bondAmount:amount,bondType:type,postedBy:'',postedDate:localToday(),status:'active'}; }
...
export function enterPlea(caseNumber:string,pleaType:string,charge:string): CourtPlea { return{caseNumber,pleaDate:localToday(),pleaType:pleaType as any,toCharge:charge,accepted:false}; }
...
export function recordSentence(caseNumber:string,sentence:string,incarceration:number,probation:number,fine:number): CourtSentencing { return{caseNumber,sentencingDate:localToday(),sentence,incarcerationDays:incarceration,probationMonths:probation,fine,restitution:0}; }
