...
export function trackMDTStatus(units:MDTStatus[]): {online:number;offline:number} { const now=Date.now();return{online:units.filter(u=>now-parseTimestamp(u.lastUpdate).getTime()<300000).length,offline:units.filter(u=>now-parseTimestamp(u.lastUpdate).getTime()>=300000).length}; }
