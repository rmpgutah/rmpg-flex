...
// v1249: PSO disposition codes — CFS disposition dropdown for pso_client_request/process_service/civil_paper_service calls now shows ONLY the 53-code PS/##.## library (10 category optgroups) instead of general police codes (RTF/ARR/etc.); general calls show no PS codes; legacy PS/055-PS/115 remain in lookup maps for backward-compat display; dispositionGroupsForIncident() updated; tests updated.
// v1248: PDF timezone fix — all process-server PDF timestamps (Notice of Attempt footer DATE/TIME, Job Sheet generation stamp, barcode reportDate) now use America/Denver (MDT/MST) via Intl API instead of OS default; addSignatureBlock consistent across timezones; en-CA ISO date format. PS-300 Section 4 uses addFieldPair (fixes garbled label row); Section 6 always renders ("No skip traces" when empty) for consecutive numbering 1–8.
...
// v1239: Training — ?session_id= alias for ?cert_id= deep-link (useRef guard+strip); duplicate ff-bulk-officer-* ids fixed (officer.id keyed per row); all 10-item checklist verified: ConfirmDialog gates record+requirement delete, N+isAdmin shortcut, Esc cascade full deps, 3-state empty states, role gates, API shape, brand tokens.
// v1238: Training Management — ?training_id=/?course_id= deep-link (useRef guard+toast+strip); ?officer_id= opens new form pre-filled; N+canWrite (admin|manager) opens New Course; Esc cascade (courseToDelete→showForm) stopPropagation full deps; ConfirmDialog gates delete; 3-state empty (loading/no-data/no-results) distinct; canWrite (admin|manager) gates create/delete; .data unwrap verified; no raw hex.
// v1239: Training — ?session_id= alias for ?cert_id= deep-link (useRef guard+strip); duplicate ff-bulk-officer-* ids fixed (officer.id keyed per row); all 10-item checklist verified: ConfirmDialog gates record+requirement delete, N+isAdmin shortcut, Esc cascade full deps, 3-state empty states, role gates, API shape, brand tokens.
...
// v1238: Training Management — ?training_id=/?course_id= deep-link (useRef guard+toast+strip); ?officer_id= opens new form pre-filled; N+canWrite (admin|manager) opens New Course; Esc cascade (courseToDelete→showForm) stopPropagation full deps; ConfirmDialog gates delete; 3-state empty (loading/no-data/no-results) distinct; canWrite (admin|manager) gates create/delete; .data unwrap verified; no raw hex.
// v1236: Text Editor — ConfirmDialog gates revert+delete (canDelete admin|manager)+N-unsaved-changes; ?document_id= deep-link alias (useRef guard+strip+toast); N+Esc cascade (newDocPendingOpen→deleteOpen→revertOpen) stopPropagation full deps; canCreate|canDelete useMemo role gates; loading/no-document distinct empty states; no raw hex.
// v1237: Training Docs — ?category= deep-link seeds category tab on mount (strip+useRef guard); ?doc_id= deep-link opens edit modal (toast+strip+useRef guard); N+isAdmin (admin|manager) opens Add Document modal; Esc cascade (docToDelete→showModal) e.stopPropagation() per branch full deps; ConfirmDialog gates delete (danger variant); 3-state empty (loading/no-data/no-results) distinct; upload+delete gated to isAdmin; API shape verified (no .data unwrap); no raw hex.
// v1235: Tasks — ConfirmDialog gates delete (canDelete admin|manager); canCreate (admin|manager|supervisor) gates New Task button+N shortcut+empty-state hint; ?task_id= deep-link (useRef guard+scroll+toast+strip); Esc cascade (deleteId→formOpen→highlightId→error→filters) stopPropagation full deps; 3-state empty (loading/no-data/no-results) distinct; .data unwrap verified; no raw hex; N shortcut deps include canCreate.
