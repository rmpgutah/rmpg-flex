...
// v1221: Recruitment — ConfirmDialog gates reject (warning) + delete (danger) applicant; GET /candidates/:id server route for deep-link fallback; ?applicant_id=/?recruit_id= deep-link (useRef guard+strip+toast); N+canManage (admin|manager|human_resources) gate; Esc cascade (rejectTarget→deleteTarget→formOpen) stopPropagation full deps; Reject Applicant button+context-menu (hidden for hired/rejected/withdrawn); 3-state empty (loading/error/no-data/no-results); brand tokens verified; XCircle icon added.
