...
// v1219: Quick Capture — ConfirmDialog gates discard (Esc with typed content → confirm before clearing); ?plate= deep-link pre-fills plate field (useRef guard+strip); canCapture (officer|dispatcher|supervisor|manager|admin) gates submit+N+inputs; canPrint (admin|manager|supervisor) gates Print FI; Esc cascade (discardConfirmOpen→result→typed-form) e.stopPropagation per branch; loading state while busy; non-capture role notice; N suppressed when confirm open; no raw hex.
