...
| No `mapWorkOrderFieldsToFleetio` mapper exists at all — raw pass-through. 16 of 19 sent fields have no Fleet.io equivalent, and the **required** `issued_at` field is never sent (RMPG sends `opened_at`, which Fleet.io doesn't recognize) | `src/utils/fleetio/sync.ts:406-414` (`dispatchOutbound`, `work_order`/`create` branch), payload built in `src/routes/workOrders.ts` emit sites | Critical — every `work_order.create` dispatch is missing a required field and should 422 on Fleet.io's side | docs/fleetio-api-reference.md:266-282 |
...
| CLAUDE.md's "Fleet.io's 50 req/min account ceiling" is not a documented Fleet.io platform limit | `CLAUDE.md` Fleet.io invariants section; self-imposed pacing lives at `src/routes/fleetio.ts:47` | Low — doc-accuracy only, not a functional bug | docs/fleetio-api-reference.md:393 |
...
**Corroborating evidence for the `archiveVendor` verb bug:** `src/utils/fleetio/sync.ts:459-478` (the `vendor`/`delete` branch of `dispatchOutbound`) calls `archiveVendor`, catches a `FleetioHttpError` with `status === 404`, calls `dropLink` to remove the `fleetio_links` row, and returns `null` — which the queue records as `status='completed'`, not `'failed'`. The live D1 query in the "Bidirectional sync status" section below shows exactly this: `outbound`/`vendor`/`delete`/`completed`, `n=3`, most recent `2026-07-29 21:53:44`. Those three "completed" rows are fully consistent with every one of those archive calls 404ing against the wrong HTTP verb (`POST` instead of the required `PATCH`) and being silently swallowed by this catch-then-drop-then-null path — there is no `status='failed'` or dead-letter trail to distinguish "archived successfully" from "404'd and gave up," because the code deliberately treats a 404 as "goal already met." That makes this bug **unrecoverable and invisible**, not just failing: a vendor whose delete event hits this path never actually gets archived on Fleet.io's side, yet its `fleetio_links` row is permanently dropped and the event is marked `completed` with no error surfaced anywhere — there is no queue state, alert, or dead-letter that would tell an operator this happened. This raises rather than lowers the urgency of the vendor-archive-verb fix (recommended ordering item #2 below): it is not merely "the operation might fail sometimes," it is "every such deletion silently and permanently fails to archive remotely while looking exactly like success."
...
| Migration `0206_fleetio_link_resource_canonicalization.sql` normalizes `fleetio_links.fleetio_resource` to the five canonical plural values | ✅ Confirmed — live `SELECT fleetio_resource, COUNT(*) FROM fleetio_links GROUP BY 1` against `785de7ae` returns `[{"fleetio_resource":"fuel_entries","COUNT(*)":22},{"fleetio_resource":"vehicles","COUNT(*)":1},{"fleetio_resource":"vendors","COUNT(*)":1}]` — every row present uses a canonical plural value; no legacy singular (`vehicle`/`vendor`) rows remain | Queried this session, `database_id=785de7ae-3e7a-4e01-93bb-d24ddd813f6b` |
