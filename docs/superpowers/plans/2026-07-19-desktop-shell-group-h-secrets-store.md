# Desktop Shell — Group H (Local Data Protection) Implementation Plan

> Initially drafted read-only (Read/Write only, no shell access) during a mid-session Bash/process-spawning infrastructure outage, grounded via direct inspection of `desktop/pinManager.js` and `desktop/localDb.js`. Finalized here on its own branch, based directly on `main` (not stacked on Group F) — Group H's scope (`desktop/pinManager.js`, `desktop/localDb.js`, new `desktop/security/secretsStore.js`) has no dependency on Group F's `desktop/security/sessionHardening.js`/`main.js` changes, only on Group G's already-merged `desktop/security/ipcGuard.js` infrastructure.
