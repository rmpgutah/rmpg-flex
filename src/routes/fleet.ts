...
fleet.get('/dashcam-videos/:id{[0-9]+}/neighbors', async (c) => {
  ...
  } catch (err) { logger.error('GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); }
});
...
fleet.get('/fuel/bulk-deliveries', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), 'SELECT * FROM fleet_fuel_log WHERE gallons > 30 ORDER BY fuel_date DESC LIMIT 200'); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
// 255: Fuel reconciliation
fleet.get('/fuel/reconciliation', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), `SELECT fr.*, v.vehicle_number FROM fleet_fuel_reconciliation fr LEFT JOIN fleet_vehicles v ON v.id = fr.vehicle_id ORDER BY fr.period_start DESC LIMIT 100`); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
...
fleet.get('/fuel/forecast', async (c) => { try { const db = getDb(c.env); const rows = await query<Record<string, unknown>>(db, "SELECT strftime(\'%Y-%m\', fuel_date) as month, SUM(gallons) as gallons, SUM(total_cost) as cost FROM fleet_fuel_log GROUP BY month ORDER BY month"); const avg = rows.length > 0 ? rows.reduce((s: number, r: any) => s + (r.gallons || 0), 0) / rows.length : 0; return c.json({ history: rows, projected_monthly_gallons: Math.round(avg), projected_monthly_cost: Math.round(avg * 3.5) }); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
// 257: Idle time estimation from fuel patterns
fleet.get('/fuel/idle-estimation', async (c) => { try { const db = getDb(c.env); const rows = await query<Record<string, unknown>>(db, `SELECT v.id, v.vehicle_number, ROUND(COALESCE(SUM(f.gallons)*0.1,0),1) as estimated_idle_hours, ROUND(COALESCE(SUM(f.gallons)*0.1*0.6,0),1) as estimated_idle_gallons FROM fleet_vehicles v LEFT JOIN fleet_fuel_log f ON f.vehicle_id = v.id AND f.fuel_date >= datetime('now', '-90 days') WHERE v.archived_at IS NULL GROUP BY v.id`); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
// 258: Alternative fuel log
fleet.get('/fuel/alt-fuel', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), `SELECT af.*, v.vehicle_number FROM fleet_alt_fuel_log af LEFT JOIN fleet_vehicles v ON v.id = af.vehicle_id ORDER BY af.created_at DESC LIMIT 200`); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
...
// 268: TSB tracking
fleet.get('/maintenance/tsbs', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), 'SELECT * FROM fleet_tsbs ORDER BY issue_date DESC LIMIT 200'); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
...
fleet.get('/service-contracts', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), `SELECT sc.*, v.vehicle_number FROM fleet_service_contracts sc LEFT JOIN fleet_vehicles v ON v.id = sc.vehicle_id ORDER BY sc.expiry_date`); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
...
fleet.get('/maintenance/:id/parts', async (c) => { try { const mid = Number(c.req.param('id')); const rows = await query<Record<string, unknown>>(getDb(c.env), 'SELECT mp.*, p.name as part_name, p.part_number FROM fleet_maintenance_parts mp LEFT JOIN fleet_parts p ON p.id = mp.part_id WHERE mp.maintenance_id = ?', mid); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
...
fleet.get('/maintenance/forecast', async (c) => { try { const db = getDb(c.env); const monthly = await query<Record<string, unknown>>(db, "SELECT strftime(\'%Y-%m\', performed_at) as month, SUM(cost) as cost FROM fleet_maintenance WHERE performed_at >= datetime('now', '-12 months') GROUP BY month ORDER BY month"); const avg = monthly.length > 0 ? monthly.reduce((s: number, r: any) => s + (r.cost || 0), 0) / monthly.length : 0; return c.json({ history: monthly, projected_monthly_cost: Math.round(avg), projected_annual_cost: Math.round(avg * 12) }); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
// 274: Recurring maintenance templates
fleet.get('/maintenance/templates', async (c) => { try { return c.json([]); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
// 275: Maintenance approval workflow
fleet.get('/maintenance/pending-approvals', async (c) => { try { return c.json([]); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
// 276: Emergency repair tracking
fleet.get('/maintenance/emergency-repairs', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), "SELECT * FROM fleet_maintenance WHERE type = 'repair' AND notes LIKE '%emergency%' ORDER BY performed_at DESC LIMIT 100"); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
// 277: Roadside assistance log
fleet.get('/roadside-assistance', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), `SELECT ra.*, v.vehicle_number FROM fleet_roadside_assistance ra LEFT JOIN fleet_vehicles v ON v.id = ra.vehicle_id ORDER BY ra.incident_date DESC LIMIT 100`); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
...
fleet.get('/maintenance/quality-checks', async (c) => { try { return c.json([]); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
// 279: Tool/equipment checkout for maintenance
fleet.get('/maintenance/tools', async (c) => { try { return c.json([]); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
// 280: Maintenance bay scheduling
fleet.get('/maintenance/bay-schedule', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), `SELECT bs.*, v.vehicle_number FROM fleet_bay_schedule bs LEFT JOIN fleet_vehicles v ON v.id = bs.vehicle_id ORDER BY bs.scheduled_start LIMIT 100`); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
...
fleet.get('/lifecycle/disposals', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), `SELECT fd.*, v.vehicle_number, v.make, v.model, v.year FROM fleet_disposals fd LEFT JOIN fleet_vehicles v ON v.id = fd.vehicle_id ORDER BY fd.disposal_date DESC`); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
...
fleet.get('/lifecycle/leases', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), `SELECT fl.*, v.vehicle_number, v.make, v.model FROM fleet_leases fl LEFT JOIN fleet_vehicles v ON v.id = fl.vehicle_id ORDER BY fl.end_date`); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
...
fleet.get('/compliance/fmcsa', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), `SELECT fc.*, v.vehicle_number FROM fleet_fmcsa_compliance fc LEFT JOIN fleet_vehicles v ON v.id = fc.vehicle_id ORDER BY fc.next_audit_due`); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
...
fleet.get('/compliance/dot-inspections', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), `SELECT v.id, v.vehicle_number, fc.annual_inspection_due, fc.annual_inspection_completed, CASE WHEN fc.annual_inspection_due IS NOT NULL AND date(fc.annual_inspection_due) < date('now') THEN 'overdue' WHEN fc.annual_inspection_due IS NOT NULL AND date(fc.annual_inspection_due) <= date('now','+30 days') THEN 'due_soon' ELSE 'current' END as status FROM fleet_vehicles v LEFT JOIN fleet_fmcsa_compliance fc ON fc.vehicle_id = v.id WHERE v.archived_at IS NULL ORDER BY status`); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
// 293: IFTA fuel tax data
fleet.get('/compliance/ifta', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), `SELECT fi.*, v.vehicle_number FROM fleet_ifta_data fi LEFT JOIN fleet_vehicles v ON v.id = fi.vehicle_id ORDER BY fi.year DESC, fi.quarter DESC LIMIT 200`); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
...
fleet.get('/compliance/defects', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), `SELECT dr.*, v.vehicle_number FROM fleet_defect_reports dr LEFT JOIN fleet_vehicles v ON v.id = dr.vehicle_id ORDER BY dr.reported_date DESC LIMIT 200`); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
...
fleet.get('/compliance/safety-equipment', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), `SELECT se.*, v.vehicle_number FROM fleet_safety_equipment se LEFT JOIN fleet_vehicles v ON v.id = se.vehicle_id ORDER BY se.next_inspection_due`); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
...
fleet.get('/compliance/load-compliance', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), `SELECT lc.*, v.vehicle_number FROM fleet_load_compliance lc LEFT JOIN fleet_vehicles v ON v.id = lc.vehicle_id ORDER BY lc.compliance_status`); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
...
fleet.get('/compliance/dvir-summary', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), `SELECT c.vehicle_id, v.vehicle_number, COUNT(*) as checks, SUM(CASE WHEN c.status='completed' THEN 1 ELSE 0 END) as passed, SUM(CASE WHEN c.status='failed' THEN 1 ELSE 0 END) as failed FROM fleet_pretrip_checklists c LEFT JOIN fleet_vehicles v ON v.id = c.vehicle_id GROUP BY c.vehicle_id ORDER BY failed DESC LIMIT 50`); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
fleet.get('/compliance/stats', async (c) => { try { const db = getDb(c.env); const vehicles = (await queryFirst<{ n: number }>(db, 'SELECT COUNT(*) as n FROM fleet_vehicles WHERE archived_at IS NULL'))?.n ?? 0; const compliant = (await queryFirst<{ n: number }>(db, "SELECT COUNT(*) as n FROM fleet_fmcsa_compliance WHERE safety_rating = 'satisfactory'"))?.n ?? 0; return c.json({ total: vehicles, compliant, non_compliant: vehicles - compliant, compliance_rate: vehicles > 0 ? Math.round((compliant / vehicles) * 100) : 0 }); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });

// ── FINANCIAL MANAGEMENT (Features 306-320) ─────────────────
// 306: Cost center allocation
fleet.get('/financial/cost-centers', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), 'SELECT * FROM fleet_cost_centers ORDER BY name'); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
...
fleet.get('/financial/cost-allocations', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), `SELECT ca.*, v.vehicle_number, cc.name as cost_center_name FROM fleet_cost_allocations ca LEFT JOIN fleet_vehicles v ON v.id = ca.vehicle_id LEFT JOIN fleet_cost_centers cc ON cc.id = ca.cost_center_id ORDER BY ca.effective_date DESC`); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
...
fleet.get('/financial/audit-trail', async (c) => { try { const db = getDb(c.env); const q = c.req.query(); const days = Math.min(Math.max(Number(q.days ?? 90), 1), 365); const fuel = await query<Record<string, unknown>>(db, `SELECT 'fuel' as type, f.id, v.vehicle_number, f.fuel_date as event_date, f.total_cost as amount, 'Fuel purchase' as description FROM fleet_fuel_log f LEFT JOIN fleet_vehicles v ON v.id = f.vehicle_id WHERE f.fuel_date >= datetime('now', '-${days} days') ORDER BY f.fuel_date DESC LIMIT 200`); const maint = await query<Record<string, unknown>>(db, `SELECT 'maintenance' as type, m.id, v.vehicle_number, m.performed_at as event_date, m.cost as amount, m.description FROM fleet_maintenance m LEFT JOIN fleet_vehicles v ON v.id = m.vehicle_id WHERE m.performed_at >= datetime('now', '-${days} days') ORDER BY m.performed_at DESC LIMIT 200`); return c.json([...fuel, ...maint].sort((a: any, b: any) => (b.event_date || '').localeCompare(a.event_date || '')).slice(0, 200)); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });

// ── OPERATIONS (Features 321-335) ───────────────────────────
// 321: Vehicle pool reservation
fleet.get('/operations/pool-reservations', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), `SELECT pr.*, v.vehicle_number FROM fleet_pool_reservations pr LEFT JOIN fleet_vehicles v ON v.id = pr.vehicle_id ORDER BY pr.reservation_start LIMIT 200`); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
...
fleet.get('/operations/upfits', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), `SELECT u.*, v.vehicle_number FROM fleet_upfits u LEFT JOIN fleet_vehicles v ON v.id = u.vehicle_id ORDER BY u.install_date DESC LIMIT 200`); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
...
fleet.get('/operations/detailing-log', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), `SELECT dl.*, v.vehicle_number FROM fleet_detailing_log dl LEFT JOIN fleet_vehicles v ON v.id = dl.vehicle_id ORDER BY dl.scheduled_date LIMIT 100`); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
...
fleet.get('/data/emissions-tests', async (c) => { try { const db = getDb(c.env); const rows = await query<Record<string, unknown>>(db, `SELECT v.id, v.vehicle_number, v.year, v.current_mileage, CASE WHEN v.year <= CAST(strftime(\'%Y\', 'now') AS INTEGER) - 6 THEN 1 ELSE 0 END AS test_due FROM fleet_vehicles v WHERE v.archived_at IS NULL AND v.year IS NOT NULL AND v.year <= CAST(strftime(\'%Y\', 'now') AS INTEGER) - 4 ORDER BY v.year`); return c.json(rows.map((r: any) => ({ ...r, test_due: !!r.test_due }))); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
...
fleet.get('/drivers/certifications', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), `SELECT dc.*, u.full_name FROM fleet_driver_certs dc LEFT JOIN users u ON u.id = dc.user_id ORDER BY dc.expiry_date`); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
...
fleet.get('/drivers/incidents', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), `SELECT di.*, u.full_name, v.vehicle_number FROM fleet_driver_incidents di LEFT JOIN users u ON u.id = di.user_id LEFT JOIN fleet_vehicles v ON v.id = di.vehicle_id ORDER BY di.incident_date DESC LIMIT 200`); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
...
fleet.get('/drivers/feedback', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), `SELECT df.*, u.full_name, v.vehicle_number FROM fleet_driver_feedback df LEFT JOIN users u ON u.id = df.user_id LEFT JOIN fleet_vehicles v ON v.id = df.vehicle_id ORDER BY df.created_at DESC LIMIT 200`); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
...
fleet.get('/risk/theft-reports', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), `SELECT ft.*, v.vehicle_number FROM fleet_vehicle_theft ft LEFT JOIN fleet_vehicles v ON v.id = ft.vehicle_id ORDER BY ft.theft_date DESC`); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
...
fleet.get('/procurement/specs', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), 'SELECT * FROM fleet_vehicle_specs WHERE is_active = 1 ORDER BY name'); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
...
fleet.get('/procurement/orders', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), `SELECT po.*, s.name as spec_name FROM fleet_procurement_orders po LEFT JOIN fleet_vehicle_specs s ON s.id = po.spec_id ORDER BY po.order_date DESC`); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
...
fleet.get('/procurement/orders/:id/bids', async (c) => { try { const orderId = Number(c.req.param('id')); const rows = await query<Record<string, unknown>>(getDb(c.env), 'SELECT * FROM fleet_vendor_bids WHERE procurement_order_id = ? ORDER BY bid_amount', orderId); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
...
fleet.get('/decommissioning/checklist/:vehicleId', async (c) => { try { const vehicleId = Number(c.req.param('vehicleId')); const rows = await query<Record<string, unknown>>(getDb(c.env), 'SELECT * FROM fleet_decommissioning WHERE vehicle_id = ? ORDER BY decommission_date DESC', vehicleId); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
...
fleet.get('/decommissioning/active', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), `SELECT fd.*, v.vehicle_number, v.make, v.model, v.year FROM fleet_decommissioning fd LEFT JOIN fleet_vehicles v ON v.id = fd.vehicle_id WHERE (fd.equipment_stripped = 0 OR fd.data_wiped = 0 OR fd.environmental_cleared = 0 OR fd.disposal_method IS NULL) ORDER BY fd.decommission_date`); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
fleet.get('/decommissioning/salvage-summary', async (c) => { try { const db = getDb(c.env); const total = (await queryFirst<{ n: number; value: number }>(db, 'SELECT COUNT(*) as n, COALESCE(SUM(salvage_value),0) as value FROM fleet_decommissioning WHERE salvage_value IS NOT NULL')) ?? { n: 0, value: 0 }; return c.json({ total_salvaged: total.n, total_salvage_value: total.value, avg_salvage: (total.n as number) > 0 ? Math.round((total.value as number) / (total.n as number)) : 0 }); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
fleet.get('/decommissioning/disposal-methods', async (c) => { try { const rows = await query<Record<string, unknown>>(getDb(c.env), 'SELECT disposal_method, COUNT(*) as count FROM fleet_decommissioning WHERE disposal_method IS NOT NULL GROUP BY disposal_method ORDER BY count DESC'); return c.json(rows); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
fleet.get('/decommissioning/reduction-analysis', async (c) => { try { const db = getDb(c.env); const active = (await queryFirst<{ n: number }>(db, "SELECT COUNT(*) as n FROM fleet_vehicles WHERE status = 'in_service' AND archived_at IS NULL"))?.n ?? 0; const aging = (await queryFirst<{ n: number }>(db, 'SELECT COUNT(*) as n FROM fleet_vehicles WHERE year < 2018 AND archived_at IS NULL'))?.n ?? 0; const highMileage = (await queryFirst<{ n: number }>(db, 'SELECT COUNT(*) as n FROM fleet_vehicles WHERE current_mileage > 120000 AND archived_at IS NULL'))?.n ?? 0; const retireable = aging + highMileage; return c.json({ active_fleet: active, candidates_for_retirement: Math.min(retireable, active), aging_vehicles: aging, high_mileage: highMileage, recommended_reduction: Math.max(0, active - Math.round(active * 0.8)) }); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
...
fleet.get('/decommissioning/stats', async (c) => { try { const db = getDb(c.env); const total = (await queryFirst<{ n: number; value: number }>(db, 'SELECT COUNT(*) as n, COALESCE(SUM(salvage_value),0) as value FROM fleet_decommissioning')) ?? { n: 0, value: 0 }; const completed = (await queryFirst<{ n: number }>(db, 'SELECT COUNT(*) as n FROM fleet_decommissioning WHERE disposal_method IS NOT NULL'))?.n ?? 0; return c.json({ total: total.n, completed, salvage_recovered: total.value, avg_days_to_complete: 0 }); } catch (err) { logger.error('fleet GET failed', { src: 'src/routes/fleet.ts' }, err); return c.json({ error: 'Failed' }, 500); } });
