...
calls.post('/:id/status', async (c) => {
  try {
    ...
    // calls_for_service.status CHECK enum. That migration ships in THIS PR, and
    // deploy.yml applies migrations before the Worker deploys, so the enum and
    // this handler stay in sync on merge.
...
// CHECK enum). Records when the hold began via status_changed_at so resume can
// show how long the call was held.
