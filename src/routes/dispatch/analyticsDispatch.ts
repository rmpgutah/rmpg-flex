...
analyticsDispatch.get('/incident-types', requireRole(...READ_ROLES), async (c) => {
  ...
  try {
    ...
    log.error('[analyticsDispatch] GET /incident-types failed', { days }, err as Error);
    return c.json({ error: 'Incident type analytics unavailable' }, 500);
  }
});
