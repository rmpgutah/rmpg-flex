...
crisis.get('/incidents', async (c) => {
  ...
  catch (err) { return c.json({ error: 'Failed to fetch crisis incidents', detail: (err as Error)?.message }, 500); }
});
...
crisis.delete('/incidents/:id', async (c) => {
  ...
  catch (err) { return c.json({ error: 'Failed to delete crisis incident', detail: (err as Error)?.message }, 500); }
});
...
crisis.get('/stats', async (c) => {
  ...
  catch (err) { return c.json({ error: 'Failed to fetch crisis stats', detail: (err as Error)?.message }, 500); }
});
