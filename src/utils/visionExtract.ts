...
/**
 * Paid-vision OCR with Claude→OpenAI fallback. Workers AI vision is handled
 * separately by extractVisionWorkersAI() below (route-level fallback chain),
 * so this restricts the router to the paid tiers and returns null when neither
 * paid provider is reachable — callers then invoke the Workers AI path.
 */
export async function extractVision(
  ...
  try {
    ...
    return normalizeVision(parsed, sel, `${r.provider}:${r.model}`, Date.now() - started);
  } catch {
    return null;
  }
}
