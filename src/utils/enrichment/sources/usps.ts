...
export async function search(seed: EnrichmentSeed, env: Bindings): Promise<SourceResult> {
  ...
  try {
    const xml = `<AddressValidateRequest USERID="${userId}"><Revision>1</Revision><Address ID="0"><Address1></Address1><Address2>${seed.address}</Address2><City>${seed.city ?? ''}</City><State>${seed.state ?? ''}</State><Zip5></Zip5><Zip4></Zip4></Address></AddressValidateRequest>`;
