    ...
    for (const ev of page.items) {
      ...
      for (const mo of ev.mediaObject) {
        ...
        if (matchChannel && mo.type === 'VIDEO') {
          // Trigger clips are ~20s driving-event clips auto-generated by the camera
          // (hard brake, speeding, etc.). They must remain as footage_markers only —
          // substituting one for our 40s on-demand segment produces the wrong duration,
          // a gap in coverage, and overlap with adjacent chunks. Skip them here; their
          // event label is already captured by buildFootageMarkers.
          if (mo.eventType && classifyDrivingEvent(mo.eventType).type !== 'custom') continue;
          // Duration guard: if ClearPath declares a clip duration, skip clips
          // significantly shorter than our requested window (catches unlabelled short
          // clips and any unrecognized event types that still produce a short clip).
          const expectedSec = (handle.toTs - handle.fromTs) / 1000;
          if (mo.durationSec != null && mo.durationSec < expectedSec * 0.75) continue;
