...
export const CORPORATE_ENHANCERS: CorporateEnhancer[] = [
  ...
  { id: 51, feature: 'Dispatch aggregates corporate strip', change: 'GET /dispatch adds clocked_in, duty_miles_today, serve_attempts_today; ShiftStatsBar renders them.', benefit: 'Dispatch board sees time/mileage without opening HR.' },
  { id: 52, feature: 'Map on-duty GPS only helper', change: 'Map toolbar shows CorporateLinkageStrip from snapshot (clocked, duty miles, serve, fleet due).', benefit: 'Map can hide off-duty ghosts using the same clock source as CAD.' },
  { id: 53, feature: 'Process server Assign clocked-in', change: 'Serve assign dropdown prefers GET /corporate-ops/servers (on duty + vehicle + miles).', benefit: 'Assigning jobs prefers officers who are actually on duty with a car.' },
