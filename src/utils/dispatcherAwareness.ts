...
async function createCall(env: Bindings, db: D1Database, req: ActionRequest): Promise<ActionResult | null> {
  ...
  if (!res.meta.last_row_id) return null;
  // Live board: the radio-born call appears on every dispatcher console the
  // instant it's minted — deduped client-side against the ~20s poll.
  const callRow = await boardCallRow(db, Number(res.meta.last_row_id));
  if (callRow) await broadcastBoard(env, 'call_created', { call: callRow });
